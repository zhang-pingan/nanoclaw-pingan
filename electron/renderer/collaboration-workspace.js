...
export function createCollaborationWorkspace(options) {
  ...
  const renderWorkItemDetail = (item) => {
    ...
    return `<section class="collaboration-detail-toolbar"><button type="button" class="btn-ghost" data-collaboration-action="close-work-item">Back</button>${mutable ? '<button type="button" class="btn-ghost" data-collaboration-action="post-work-progress">Post progress</button>' : ''}</section>
      <section class="collaboration-section"><div class="collaboration-section-head"><h3>${html(item.title)}</h3>${status(item.status)}</div><p class="collaboration-prose">${html(item.description || '')}</p><dl class="collaboration-definition-list"><div><dt>Owner</dt><dd>${html(item.owner_principal_id)}</dd></div><div><dt>Priority</dt><dd>${html(item.priority)}</dd></div><div><dt>Due</dt><dd>${html(timestamp(item.due_at))}</dd></div><div><dt>Assignment</dt><dd>${html(item.assignment_status)}</dd></div><div><dt>Labels</dt><dd>${(item.labels || []).map((label) => `<code>${html(label)}</code>`).join(' ') || '-'}</dd></div><div><dt>Blocked by</dt><dd>${(item.blocked_by || []).map((id) => `<code>${html(id)}</code>`).join(' ') || '-'}</dd></div></dl>${mutable ? `<div class="collaboration-segmented">${['proposed', 'open', 'in_progress', 'blocked', 'done', 'cancelled'].map((value) => `<button type="button" data-collaboration-action="set-work-status" data-status="${value}" class="${item.status === value ? 'active' : ''}">${html(value)}</button>`).join('')}</div>` : ''}</section>
