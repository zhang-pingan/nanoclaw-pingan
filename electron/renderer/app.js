...
async function collaborationRequest(path, requestOptions = {}) {
  const multipart = requestOptions.body instanceof FormData;
  const response = await fetch(`${API_BASE_URL}/api/collaboration${path}`, {
    ...requestOptions,
    headers: {
  ...
  if (!response.ok)
    throw new Error(
      data && typeof data === 'object' && data.error
        ? data.error
        : `Collaboration HTTP ${response.status}`,
    );
  return data;
}

var collaborationWorkspace = createCollaborationWorkspace({
...
function renderCollaborationList() {
  collaborationWorkspace.renderList();
}

function renderCollaborationShell() {
  collaborationWorkspace.renderShell();
}

async function loadCollaborationGroups() {
  return collaborationWorkspace.loadGroups();
}

async function selectCollaborationGroup(groupId, options = {}) {
  return collaborationWorkspace.selectGroup(groupId, options);
}

async function loadCollaborationDetail(groupId, options = {}) {
  return collaborationWorkspace.selectGroup(groupId, {
    tab: collaborationState.activeTab,
    updateRoute: options.updateRoute,
  });
}

function openCollaborationFromNotification(groupId) {
  return collaborationWorkspace.openNotification(groupId);
}

function openCreateCollaborationDialog() {
  collaborationWorkspace.openCreate();
}

function openJoinCollaborationDialog() {
  collaborationWorkspace.openObserve();
}
...
async function syncSelectedCollaborationGroup() {
  return collaborationWorkspace.syncSelected();
}

async function handleCollaborationContentAction(button) {
  button.disabled = true;
  try {
    await collaborationWorkspace.handleAction(button);
  } catch (error) {
...
async function selectCollaborationTab(tab, options = {}) {
  return collaborationWorkspace.selectTab(tab, options);
}

async function restoreCollaborationRoute(pathname) {
  return collaborationWorkspace.restoreRoute(pathname);
}
...
function renderAssistantInboxActions(item) {
  ...
  const isExecutable = item.action_kind === 'continue_today_plan';
  return `
...
async function clearAssistantData() {
  if (
    !confirm(
      '确定清除个人助手的聊天记录、Inbox、动作日志和稍后提醒？运行设置会保留。',
    )
  ) {
    return;
  }
