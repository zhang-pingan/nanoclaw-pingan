...
目标：阅读 `sources.json` 和 `search_log.json`，抽取证据卡片、初步发现、冲突点、限制和知识缺口。你不新增来源、不新增任意 HTML、不写最终报告；本阶段是来源层和报告写作之间的证据层。
...
6. 不从 `sources.json` 之外补充新事实；发现来源不足时记录 source gap，而不是编造 evidence。
7. `findings.json` 的 claim 必须是后续 writer 可以直接引用和改写的证据支撑结论。
