...
8. 使用 `mcp__nanoclaw__ask_user_question` 向用户发起确认，优先遵循以下规则：
...
9. **等待用户回复**（工具会阻塞直到收到回答、超时或被拒绝），根据结果继续提问或进入方案生成
10. 如果用户回复解答了所有疑问，进入步骤 3；如果工具返回超时、跳过或拒绝，需明确评估是否还能继续，不能继续时按失败结果调用 `complete_delegation`
