...
const BUILTIN_TOOL_VISIBILITY: Record<string, ToolVisibility> = {
  ...
  ios_app_prepare_session: 'all',
  ios_app_observe: 'all',
  ios_app_act: 'all',
  ios_app_run_flow: 'all',
  ios_app_read_trace: 'all',
  ios_app_search_code: 'all',
  ios_app_write_claims: 'all',
  ios_app_run_test_case: 'all',
  ios_app_write_report: 'all',
  ios_host_debug_shell: 'all',
  reload_tools: 'all',
};
...
async function callIosAppRequestTool(
  action: string,
  args: Record<string, unknown>,
  waitMs = 300000,
) {
  const requestId = `iosapp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  if (!result) {
    return {
      content: [
        {
          type: 'text' as const,
          text: `${action} timed out waiting for response (requestId=${requestId}).`,
        },
      ],
      isError: true,
    };
  ...
  const errorMessage =
    typeof result.error === 'string'
      ...
      : result.error && typeof result.error === 'object'
        ...
        : '';
  const isError =
    result.status === 'error' || result.status === 'blocked' || !!errorMessage;
  return {
    content: [
      {
        type: 'text' as const,
...
const iosActionStepSchema = z
  .object({
    ...
    wait_for: z.record(z.string(), z.unknown()).optional(),
  })
  .passthrough();

const iosAssertionSchema = z.record(z.string(), z.unknown());

server.tool(
  'ios_app_prepare_session',
  '创建 iOS Simulator App 操作会话。读取 services.json 的 clients.ios 配置，启动模拟器，可选构建/安装/启动 App，并写入 SESSION/BUILD/STATE evidence。缺少 Xcode、Simulator、客户端配置或仓库时返回结构化环境错误，不伪造成功。',
  {
    service: z.string().min(1).describe('services.json 中的服务名'),
    purpose: z
      .string()
      .optional()
      .default('product_recon')
      .describe('会话用途，例如 product_recon 或 acceptance'),
...
server.tool(
  'ios_app_observe',
  '观察当前 iOS App 状态，返回 screenshot/UI tree/可交互元素/app_state/network_cursor，并写入 OBS/SCREENSHOT/UI_TREE evidence。默认连接 Appium http://127.0.0.1:4723。',
  {
...
server.tool(
  ...
  '执行 iOS App 动作并自动关联动作前后 observe。支持 tap/type/scroll/back/home/deeplink/wait/terminate/relaunch。',
  {
    ...
    clear: z.boolean().optional(),
    wait_for: z.record(z.string(), z.unknown()).optional(),
    snapshot_after: z.boolean().optional().default(true),
    launch_args: z.array(z.string()).optional(),
  },
  async (args) => callIosAppRequestTool('act', args as Record<string, unknown>),
);

server.tool(
  'ios_app_run_flow',
  '执行调用方提供的可复现 iOS flow。每个 step 会走 ios_app_act，并生成 FLOW evidence。',
  {
    session_id: z.string().min(1),
...
server.tool(
  'ios_app_read_trace',
  '读取当前 session 的 iOS 网络 JSONL / App log / crash trace。第一版网络日志来自 clients.ios.automation.network_log_path，并输出脱敏 NET evidence。',
  {
    session_id: z.string().min(1),
    after_action: z.string().optional(),
    types: z.array(z.enum(['network', 'app_log', 'crash'])).optional(),
    filters: z
      .object({
...
server.tool(
  ...
  '把 agent 提炼的 claim 写入 evidence store。底座只分配 CLAIM id、落盘，并校验 supported_by 引用的 evidence 必须存在于本 session。',
  {
    session_id: z.string().min(1),
    claims: z
      .array(
        z.object({
          ...
          supported_by: z.array(z.string().min(1)).min(1),
...
server.tool(
  ...
  '执行调用方给定的 iOS test case steps 和 assertions，返回结构化 case/assertion/evidence。它不生成测试计划内容。',
  {
    session_id: z.string().min(1),
...
server.tool(
  ...
  '通用 iOS JSON 产物写盘出口。写盘前统一脱敏，校验 required_fields，并校验 body 中 FLOW/NET/CLAIM 等 evidence 引用真实存在。',
  {
    session_id: z.string().min(1),
    kind: z.string().min(1),
    path: z.string().min(1),
...
server.tool(
  ...
  'iOS harness 调试逃生口：在宿主机执行任意 shell 命令用于排查 xcrun/xcodebuild/Appium/日志问题。返回永远标记 usable_as_formal_evidence=false，不能直接支撑正式 claim 或 acceptance passed。',
  {
    ...
    capture: z
      .object({
        ...
        max_bytes: z.number().int().min(1000).max(100000).optional(),
      })
      .optional(),
  },
      ...
      const summary = {
        ...
        image: result.image
          ? {
              ...result.image,
              data: undefined,
              dataBase64Chars: imageDataLength,
            }
          : undefined,
        nextSteps: result.image?.containerPath
          ? {
              sendToChat: `call send_file with file_path="${result.image.containerPath}"`,
              inspectImage: `call Read with file_path="${result.image.containerPath}"`,
            }
          : undefined,
      };
