...
当前工程统一采用 **latest-only** 开发版本策略。由于尚无需要保留的真实历史业务数据或外部客户端兼容面，协议、JSON Schema、API、事件模型、Git 目录和 SQLite Schema 每次只实现最新版本：旧 parser/writer/reducer、迁移链、双写、兼容回放和旧 fixture 随替代版本一起删除；版本不匹配在边界处 fail closed，开发环境通过精确、显式的 reset 或 reinitialize 切换到 current version。版本字段用于确定性识别和拒绝陈旧输入，不构成兼容承诺。该策略不降低安全边界，也不允许静默删除源码、配置、凭据和用户文件；首次出现不可丢弃业务数据、签名历史或外部客户端依赖前，必须先定义正式 compatibility freeze point 和升级策略。
