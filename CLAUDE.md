...
The project-wide development version policy is **latest-only**. There is currently no real historical business dataset, signed protocol history, or external client population that requires backward compatibility. When a protocol, JSON schema, API, event model, Git layout, or SQLite schema changes, implement only the new current version and remove superseded readers, writers, reducers, endpoints, migrations, dual-write paths, fixtures, and compatibility tests. Keep explicit version fields to reject stale or future input fail closed; they are not compatibility promises. Handle stale development state with an exact, explicit reset or reinitialization, never by silently deleting source, configuration, credentials, user-authored files, or unrelated state. Before irreplaceable data or an external client dependency exists, define a compatibility freeze point and a new migration/version-support policy.
...
- When changing collaboration behavior, treat Project Space protocol v3 and `collaboration.db` schema v7 as the only current model. Preserve system-generated Group-scoped Principals, installation-scoped Clients, per-Client Icarus event Credentials, direct permissions, the strict Observer request allowlist, verified Git replay, object revisions, Turn fencing, durable receipts, staged Artifact integrity, timeout notifications, and audit redaction. Git Remote SSH access is transport-only and never establishes a Principal or Icarus permission. Do not restore fingerprint-derived Principals, `signingKeyPath` identity logic, Role/Claim, a Group-level Machine/active Turn, old schema readers, migrations, dual writes, or legacy replay.
