...
- When changing collaboration behavior, treat Project Space protocol v3 and `collaboration.db` schema v6 as the only current model. Preserve Principal/Client separation, direct permissions, Observer read-only behavior, verified Git replay, object revisions, Turn fencing, durable receipts, staged Artifact integrity, timeout notifications, and audit redaction. Do not restore Role/Claim, a Group-level Machine/active Turn, v2 readers, migrations, dual writes, or legacy replay.
