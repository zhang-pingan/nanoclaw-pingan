...
Adds Parallel AI MCP integration to Icarus for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Icarus scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Icarus already set up and running
...
Add Parallel AI MCP servers after the icarus server:
```typescript
const mcpServers: Record<string, any> = {
  icarus: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__icarus__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__icarus__send_message
   4. Use mcp__icarus__complete_scheduled_task to mark this task as done
...
```bash
sleep 3
launchctl list | grep icarus  # macOS
# Linux: systemctl --user status icarus.service
```
...
- Check task runs: `tail -f logs/icarus.log | grep "scheduled task"`
