...
const CREATION_WORKFLOW_CONTEXT_TABLES: readonly TableSeed[] = [
  ...
  },
  {
    ...
    columns: [
      ...
      text('terminal_kind', true, ['normal', 'errored', 'cancelled']),
    ...
    checks: [
      ...
      check(
        ...
        'normal, errored and cancelled terminal activations populate mutually exclusive fields',
      ),
      check(
...
const GRAPH_FACT_AND_EFFECT_TABLES: readonly TableSeed[] = [
  ...
  {
    name: 'workflow_provider_cancellation_requests',
    sourceSection: 'Edge Resolution, Candidate and Cut',
    columns: [
      id('id'),
      id('graph_run_id'),
    ...
    foreignKeys: [
      fk(
        'fk:provider_cancellations:attempt',
        ['graph_run_id', 'scope_id', 'node_id', 'attempt_id'],
        'workflow_graph_node_attempts',
        ['graph_run_id', 'scope_id', 'node_id', 'id'],
      ),
      fk(
        'fk:provider_cancellations:effect',
        'effect_operation_id',
        'workflow_graph_effect_operations',
      ),
      fk('fk:provider_cancellations:outbox', 'outbox_id', 'workflow_outbox'),
      fk(
        'fk:provider_cancellations:close_request',
        ['graph_run_id', 'scope_id', 'close_request_id'],
        'workflow_graph_scope_close_requests',
        ['graph_run_id', 'scope_id', 'id'],
      ),
      registryFk('fk:provider_cancellations:adapter', 'adapter'),
    ],
    uniqueKeys: [
      uk('uk:provider_cancellations:attempt', ['attempt_id']),
      uk('uk:provider_cancellations:external_execution', [
        'external_execution_id',
      ]),
    ],
    pairedColumns: [
    ...
    indexes: [
      index(
        'idx:provider_cancellations:due',
        'recovery',
        ['status', 'next_attempt_at_ms', 'lease_expires_at_ms', 'id'],
        "status in ('requested','processing','retry_wait')",
        ['query:provider_cancellation_due'],
      ),
    ],
  },
  {
...
export const LOGICAL_QUERY_INTENTS: readonly LogicalQueryIntent[] = [
  ...
  query(
    'query:provider_cancellation_due',
    'recovery',
    'claim requested, retryable, or expired provider cancellation work',
    'workflow_provider_cancellation_requests',
    'idx:provider_cancellations:due',
    {
      equality: ['status'],
      range: ['next_attempt_at_ms', 'lease_expires_at_ms'],
      predicate: "status in ('requested','processing','retry_wait')",
      orderBy: [['id', 'asc']],
    },
  ),
  query(
