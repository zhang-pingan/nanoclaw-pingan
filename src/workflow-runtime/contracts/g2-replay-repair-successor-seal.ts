...
import {
  ...
  G2_REPLAY_REPAIR_DRAFT_CASES_REF,
  G2_REPLAY_REPAIR_DRAFT_INVENTORY_REF,
  G2_REPLAY_REPAIR_DRAFT_MANIFEST_REF,
  G2_REPLAY_REPAIR_REVIEW_REPORT_REF,
  G2_REPLAY_REPAIR_SEALED_ROOT,
  G2_REPLAY_REPAIR_SEMANTIC_REVIEW_REF,
...
export const G2_REPLAY_REPAIR_SEALED_BUNDLE_REF = `${G2_REPLAY_REPAIR_SEALED_ROOT}/golden-conformance-bundle@2.json`;
export const G2_REPLAY_REPAIR_SEALED_INVENTORY_REF = `${G2_REPLAY_REPAIR_SEALED_ROOT}/artifact-inventory@2.json`;
export const G2_REPLAY_REPAIR_SEALED_BUNDLE_SCHEMA_REF = `${G2_REPLAY_REPAIR_SEALED_ROOT}/schemas/golden-conformance-bundle-schema@2.json`;
...
const DRAFT_ARTIFACT_HASH =
  'sha256:164f6f0962e2005f8e4c3573aa9f9f14d3eba62322ae464eca927ac2eae98fc9';
const REVIEW_REPORT_ARTIFACT_HASH =
  'sha256:781d33faefa4d77d210d293ffea97cc9c712f2a3c23690abf4f2ead135f43435';
...
export interface G2ReplayRepairSealBuild {
  files: Map<string, string>;
  inventory: ContractArtifactEnvelope;
  bundle: ContractArtifactEnvelope;
}

export class G2ReplayRepairSealError extends Error {
  readonly code = 'g2_replay_repair_seal_error';

  constructor(message: string) {
    super(message);
    this.name = 'G2ReplayRepairSealError';
  }
}
...
function absolute(relativePath: string): string {
  ...
  if (!resolved.startsWith(`${contractsRoot}${path.sep}`)) {
    throw new G2ReplayRepairSealError(
...
function object(value: JsonValue, label: string): JsonObject {
  try {
    ...
    throw new G2ReplayRepairSealError(`Expected object: ${label}`);
  }
}

function objects(value: JsonValue, label: string): JsonObject[] {
  if (!Array.isArray(value)) {
    throw new G2ReplayRepairSealError(`Expected array: ${label}`);
  }
...
function string(value: JsonValue, label: string): string {
  if (typeof value !== 'string') {
    throw new G2ReplayRepairSealError(`Expected string: ${label}`);
  }
  return value;
}
...
function strings(value: JsonValue, label: string): string[] {
  if (
    ...
    throw new G2ReplayRepairSealError(`Expected string array: ${label}`);
  }
...
function requireInternalHash(
  ...
  if (
    ...
    throw new G2ReplayRepairSealError(`${label} internal hash drift`);
  }
}
...
function sealedInputRef(caseId: string, kind: 'source' | 'snapshot'): string {
  return `${G2_REPLAY_REPAIR_SEALED_ROOT}/inputs/${caseId}.${
    kind === 'snapshot' ? 'snapshot@2' : 'source'
  }.json`;
}

function sealedExpectedRef(originalRef: string): string {
  return `${G2_REPLAY_REPAIR_SEALED_ROOT}/expected/${path.basename(originalRef)}`;
}
...
function validateApprovalChain(): {
  ...
  const review = readArtifact(G2_REPLAY_REPAIR_SEMANTIC_REVIEW_REF);
  const checkedReview = checkG2ReplayRepairSemanticReview();
  requireInternalHash(
    draft.payload,
    'draft_manifest_hash',
    DRAFT_DOMAIN,
...
export function buildG2ReplayRepairSeal(): G2ReplayRepairSealBuild {
  ...
  for (const draftCase of cases) {
    ...
    try {
      ...
      throw new G2ReplayRepairSealError(`Raw source is not UTF-8: ${caseId}`);
    }
    ...
    if (
      ...
      throw new G2ReplayRepairSealError(`Raw source identity drift: ${caseId}`);
    }
    ...
    sealedCases.push({
      ...
      approved_review_ref: G2_REPLAY_REPAIR_SEMANTIC_REVIEW_REF,
  ...
  const bundleSchema = artifact(
    'icarus.workflow-compiler-conformance-bundle-schema/1',
    'icarus.workflow-compiler-conformance-bundle-schema.g2-production-compiler-replay-repair-v2',
    BUNDLE_SCHEMA_DOMAIN,
    G2_REPLAY_REPAIR_CONFORMANCE_BUNDLE_SCHEMA,
  );
  const inventorySchema = artifact(
    'icarus.workflow-compiler-conformance-inventory-schema/1',
    'icarus.workflow-compiler-conformance-inventory-schema.g2-production-compiler-replay-repair-v2',
    INVENTORY_SCHEMA_DOMAIN,
    G2_REPLAY_REPAIR_SEALED_INVENTORY_SCHEMA,
  );
  for (const [schemaRef, schemaArtifact] of [
    [G2_REPLAY_REPAIR_SEALED_BUNDLE_SCHEMA_REF, bundleSchema],
    [G2_REPLAY_REPAIR_SEALED_INVENTORY_SCHEMA_REF, inventorySchema],
  ] as const) {
  ...
  const validateInventory = new Ajv2020({
    strict: true,
    allErrors: true,
  }).compile(G2_REPLAY_REPAIR_SEALED_INVENTORY_SCHEMA as AnySchema);
  ...
  const inventory = artifact(
    ...
    'icarus.workflow-compiler-conformance-inventory.g2-production-compiler-replay-repair-v2',
    INVENTORY_ARTIFACT_DOMAIN,
    inventoryPayload,
  );
  files.set(G2_REPLAY_REPAIR_SEALED_INVENTORY_REF, render(inventory));
  ...
  const bundleWithoutHash = {
    ...
    golden_semantic_review_ref: G2_REPLAY_REPAIR_SEMANTIC_REVIEW_REF,
    ...
    cases: sealedCases,
    inventory_ref: G2_REPLAY_REPAIR_SEALED_INVENTORY_REF,
  ...
  const validateBundle = new Ajv2020({
    strict: true,
    allErrors: true,
  }).compile(G2_REPLAY_REPAIR_CONFORMANCE_BUNDLE_SCHEMA as AnySchema);
  ...
  const bundle = artifact(
    ...
    'icarus.workflow-compiler-conformance.g2-production-compiler-replay-repair-v2',
    BUNDLE_ARTIFACT_DOMAIN,
    bundlePayload,
  );
  files.set(G2_REPLAY_REPAIR_SEALED_BUNDLE_REF, render(bundle));
...
function relativeWithinSeal(repositoryPath: string): string {
  const prefix = `${G2_REPLAY_REPAIR_SEALED_ROOT}/`;
...
export function generateG2ReplayRepairSeal(): ContractArtifactEnvelope {
  const built = buildG2ReplayRepairSeal();
  const root = absolute(G2_REPLAY_REPAIR_SEALED_ROOT);
...
export function checkG2ReplayRepairSeal(): ContractArtifactEnvelope {
  assertCurrentG2SealedBoundary(absolute('conformance/sealed'));
  const built = buildG2ReplayRepairSeal();
  checkFiles(absolute(G2_REPLAY_REPAIR_SEALED_ROOT), built.files);
  return built.bundle;
}
