...
export const G39_PACK_RELEASE_ACTIVATION_FORMATS = {
  request: 'icarus.workflow-pack-release-activation-request/1',
  receipt: 'icarus.workflow-pack-release-activation-receipt/1',
  result: 'icarus.workflow-pack-release-activation-result/1',
} as const;
...
export interface G39PackReleaseClaim extends JsonObject {
  release_id: string;
  ref: VersionedRef;
  hash: Sha256Hash;
...
export interface G39PackReleaseIdentity extends JsonObject {
  release_id: string;
  ref: VersionedRef;
  hash: Sha256Hash;
}

export interface G39TargetPackReleaseClaim extends G39PackReleaseClaim {
...
export type G39ExpectedPointer =
  ...
  | {
      state: 'present';
      row_version: number;
      release: G39PackReleaseIdentity;
    };
...
export interface G39PackReleaseActivationRequest extends JsonObject {
  format: typeof G39_PACK_RELEASE_ACTIVATION_FORMATS.request;
  command_type: 'activate_pack_release';
  ...
  requested_at_ms: number;
  pack_id: string;
  target_release: G39TargetPackReleaseClaim;
  previous_release: G39PackReleaseClaim | null;
...
export interface G39PackReleaseActivationReceipt extends JsonObject {
  format: typeof G39_PACK_RELEASE_ACTIVATION_FORMATS.receipt;
  command_id: string;
  domain_request_hash: Sha256Hash;
  pack_id: string;
  target_release: G39PackReleaseIdentity;
  previous_release: G39PackReleaseIdentity | null;
...
export interface G39PackReleaseActivationResult extends JsonObject {
  format: typeof G39_PACK_RELEASE_ACTIVATION_FORMATS.result;
