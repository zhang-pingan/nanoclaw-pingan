...
Historical G0/R-016/R-020/R-021/R-022 coverage, Working/Draft review artifacts, superseded Compiler authorities, and Gate provenance remain immutable under `conformance/` or Git history. They are not default Markdown inputs and are not regenerated by the current aggregate.
...
`contracts:check` verifies current foundation/schema artifacts, current Compiler and sealed replay, Registry and node-output authority, static absence, and the immutable accepted release. `test:current` exercises the current Store, Runtime, certification, and activation behavior against test-owned temporary state. Neither command reads the archived construction Markdown or recomputes historical Markdown coverage and Gate milestones.
