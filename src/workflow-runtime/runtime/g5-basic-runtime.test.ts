...
} from '../contracts/static-child-plan-bundle-fixture-harness.js';
import type {
  StaticChildPlanBundleFixtureCase,
  StaticChildPlanBundleFixtureOracle,
} from '../contracts/static-child-plan-bundle-repair.js';
import {
...
function staticChildBridgeBehavior(fixture: StaticChildPlanBundleFixtureCase): {
  readonly behavior: string;
  readonly variants: readonly string[];
} {
  requireBridge(
    fixture.input.expected_surface === fixture.surface,
    `${fixture.case_id} input surface drifted`,
  );
  requireBridge(
    fixture.input.behavior !== fixture.case_id,
    `${fixture.case_id} behavior aliases its case id`,
  );
  requireBridge(
    fixture.input.fixture_token ===
      `static-child-plan-bundle-fixture:${fixture.case_id}`,
    `${fixture.case_id} fixture token drifted`,
  );
  return {
    behavior: fixture.input.behavior,
    variants: fixture.input.variants,
  };
}

function compilerStaticChildInput(): {
...
function invokeStaticChildCompiler(
  ...
  requireBridge(
    fixture.surface === 'Compiler' &&
      fixture.operation === 'compile_workflow' &&
      fixture.handler === 'compile_workflow_production',
    `${fixture.case_id} Compiler dispatch drifted`,
  );
  const outcome = compileWorkflow({
    caseId: fixture.input.fixture_token,
...
function executeStaticChildCompilerFixture(
  fixture: StaticChildPlanBundleFixtureCase,
): StaticChildPlanBundleFixtureOracle {
  const { behavior, variants } = staticChildBridgeBehavior(fixture);
  const input = compilerStaticChildInput();
  const checks: string[] = [];
  if (behavior === 'compile_nested_closure_and_compare_fixed_parent') {
    requireBridge(
      canonicalJson([...variants]) === canonicalJson(['nested_parent']),
      `${fixture.case_id} variants drifted`,
    );
    const compiled = invokeStaticChildCompiler(
      fixture,
      input.source,
      input.snapshot,
    );
    requireBridge(
      canonicalJson(compiled.plan) === canonicalJson(input.expectedPlan),
      `${fixture.case_id} changed fixed parent Plan bytes`,
    );
    checks.push('parent_plan_canonical_bytes_exact');
    const entries = compiled.staticChildPlanBundle.entries;
    requireBridge(
      canonicalJson(entries.map((entry) => entry.closureKey)) ===
        canonicalJson(['nested', 'nested/leaf']) &&
        canonicalJson(entries.map((entry) => entry.plan.plan_hash)) ===
          canonicalJson(
            input.expectedPlan.static_child_plan_closure.members.map(
              (member) => member.plan_hash,
            ),
          ),
      `${fixture.case_id} membership, order, key, ref, or hash drifted`,
    );
    checks.push('bundle_membership_order_key_ref_hash');
    requireBridge(
      entries.every(
        (entry) =>
          domainSeparatedSha256(
            'icarus:workflow-graph-source:1\n',
            entry.source,
          ) === entry.plan.source_hash &&
          canonicalJson(entry.source) ===
            canonicalJson(JSON.parse(canonicalJson(entry.source))) &&
          canonicalJson(entry.plan) ===
            canonicalJson(JSON.parse(canonicalJson(entry.plan))),
      ),
      `${fixture.case_id} source or Plan canonical bytes drifted`,
    );
    checks.push('nested_source_and_plan_canonical_bytes');
    requireBridge(
      canonicalJson(
        entries.map((entry) => [
          entry.closureKey,
          entry.plan.static_child_plan_closure.members.map(
            (member) => member.closure_key,
          ),
        ]),
      ) ===
        canonicalJson([
      ...
      `${fixture.case_id} nested lineage drifted`,
    );
    checks.push('nested_lineage_exact');
  } else if (
    behavior === 'compile_repeated_nested_factory_and_compare_shared_descendant'
  ) {
    requireBridge(
      canonicalJson([...variants]) === canonicalJson(['shared_descendant']),
      `${fixture.case_id} variants drifted`,
    );
    ...
    requireBridge(original && done, `${fixture.case_id} source nodes missing`);
    ...
    const compiled = invokeStaticChildCompiler(fixture, source, input.snapshot);
    const entries = compiled.staticChildPlanBundle.entries;
    requireBridge(
      canonicalJson(entries.map((entry) => entry.closureKey)) ===
        canonicalJson([
      ...
      `${fixture.case_id} closure membership or order drifted`,
    );
    checks.push('closure_membership_and_order_exact');
    const leaves = entries.filter((entry) =>
      entry.closureKey.endsWith('/leaf'),
    );
    requireBridge(
      leaves.length === 2 &&
        leaves[0]!.plan.plan_hash === leaves[1]!.plan.plan_hash,
      `${fixture.case_id} descendant Plan hash was not shared`,
    );
    checks.push('shared_descendant_plan_hash');
    requireBridge(
      canonicalJson(leaves[0]!.plan) === canonicalJson(leaves[1]!.plan),
      `${fixture.case_id} shared Plan bytes drifted`,
    );
    checks.push('shared_descendant_plan_canonical_bytes');
    requireBridge(
      canonicalJson(leaves[0]!.source) === canonicalJson(leaves[1]!.source),
      `${fixture.case_id} shared source bytes drifted`,
    );
    checks.push('shared_descendant_source_canonical_bytes');
  } else {
    throw new Error(`${fixture.case_id} Compiler behavior is unhandled`);
  }
  return {
    disposition: 'compiled',
    sqlite_state: 'not_applicable',
    exact_error: null,
    checks,
  };
}

interface StaticChildT2aFixtureContext {
  readonly instance: G5TestBootstrapInstance;
  readonly seed: SeededRuntime;
  readonly compiledFixture: ReturnType<typeof staticChildBundleFixture>;
  readonly created: ReturnType<typeof createWorkflowT0>;
  readonly input: Parameters<typeof persistCompileResultT2a>[1];
}

function staticChildT2aContext(
  fixture: StaticChildPlanBundleFixtureCase,
  variant: string,
): StaticChildT2aFixtureContext {
  requireBridge(
    fixture.surface === 'T2a' &&
      fixture.operation === 'persist_compile_result_t2a' &&
      fixture.handler === 'persist_static_child_plan_bundle_t2a_production',
    `${fixture.case_id} T2a dispatch drifted`,
  );
  const instance = bootstrap(
    `${fixture.input.fixture_token}:${variant}`.replaceAll(':', '-'),
  );
  const seed = seedRuntime(instance.store);
  const compiledFixture = staticChildBundleFixture(seed);
  pinTestDefinitionPlan(instance.store, seed, compiledFixture.parentPlan);
  const created = createWorkflowT0(
    instance.store,
    creationInput(seed, `${fixture.input.fixture_token}:${variant}`, 10),
  );
  return {
    instance,
    seed,
    ...
    input: {
      ...
      sourceHash: compiledFixture.parentPlan.source_hash as Sha256Hash,
      plan: compiledFixture.parentPlan,
      staticChildPlanBundle: compiledFixture.bundle,
      nowMs: 20,
    },
  };
}

function staticChildTotalChanges(instance: G5TestBootstrapInstance): number {
  return instance.store.queryOne<{ count: number }>(
    'SELECT total_changes() AS count',
    [],
  )!.count;
}

function staticChildPlanAndBindingState(
  context: StaticChildT2aFixtureContext,
): Sha256Hash {
  const { instance, created } = context;
  return domainSeparatedSha256(
    'icarus:workflow-static-child-plan-bundle-sqlite-state:1\n',
    {
      plans: instance.store.queryAll<Record<string, JsonValue>>(
        `SELECT * FROM workflow_graph_scope_plans
          WHERE graph_run_id = ? ORDER BY plan_hash COLLATE BINARY`,
        [created.activation.graphRunId],
      ),
      contents: instance.store.queryAll<Record<string, JsonValue>>(
        `SELECT * FROM workflow_generated_schema_contents
          WHERE schema_ref IN (
            SELECT schema_ref FROM workflow_plan_generated_schemas
             WHERE graph_run_id = ?
          ) ORDER BY schema_ref COLLATE BINARY`,
        [created.activation.graphRunId],
      ),
      bindings: instance.store.queryAll<Record<string, JsonValue>>(
        `SELECT * FROM workflow_plan_generated_schemas
          WHERE graph_run_id = ?
          ORDER BY plan_hash COLLATE BINARY, schema_ref COLLATE BINARY`,
        [created.activation.graphRunId],
      ),
      build: instance.store.queryAll<Record<string, JsonValue>>(
        'SELECT * FROM workflow_graph_scope_builds WHERE id = ?',
...
function observeStaticChildError(
  action: () => unknown,
): StaticChildPlanBundleFixtureOracle['exact_error'] {
  try {
    action();
  } catch (error) {
    if (error instanceof G5RuntimeError) {
      requireBridge(
        ...
        `unexpected G5 error ${error.code}`,
      );
      return error.code as StaticChildPlanBundleFixtureOracle['exact_error'];
    }
    throw error;
  }
  throw new Error(
    'Static child fixture execution: expected production rejection',
  );
}

function requireStaticChildNoWrite(
  context: StaticChildT2aFixtureContext,
  beforeState: Sha256Hash,
  beforeChanges: number,
): void {
  requireBridge(
    staticChildPlanAndBindingState(context) === beforeState,
    'T2a durable state changed on rejection',
  );
  requireBridge(
    staticChildTotalChanges(context.instance) === beforeChanges,
    'T2a rejection executed DML',
  );
}

function replaceStaticChildPlan(
  context: StaticChildT2aFixtureContext,
  entryIndex: number,
  childPlan: CompiledScopePlanV2Document,
): {
  readonly parentPlan: CompiledScopePlanV2Document;
  readonly bundle: WorkflowCompilerStaticChildPlanBundle;
} {
  const bundle = structuredClone(context.compiledFixture.bundle);
  bundle.entries[entryIndex] = {
    ...bundle.entries[entryIndex]!,
    plan: childPlan,
  };
  const members = structuredClone(
    context.compiledFixture.parentPlan.static_child_plan_closure.members,
  );
  ...
  const rebuiltMember = {
    ...
    interface_snapshot_hash: childPlan.interface_snapshot_hash as Sha256Hash,
  };
  members[entryIndex] = {
    ...rebuiltMember,
    member_hash: domainSeparatedSha256(
  ...
  const { plan_hash: _parentHash, ...parentWithoutHash } =
    context.compiledFixture.parentPlan;
  void _parentHash;
  return {
...
function staticChildFaultPoint(
  fixture: StaticChildPlanBundleFixtureCase,
): string {
  requireBridge(fixture.fault !== null, `${fixture.case_id} fault is missing`);
  requireBridge(
    canonicalJson(fixture.fault.variants) ===
      canonicalJson(fixture.input.variants),
    `${fixture.case_id} fault variants drifted`,
  );
  return fixture.fault.point;
}

function observedStaticChildChecks(behavior: string): string[] {
  switch (behavior) {
    case 'remove_last_bundle_member':
    case 'append_excess_bundle_member':
      return ['closed_membership', 'zero_dml'];
    case 'replace_member_with_duplicate_closure_entry':
      return ['unique_membership', 'zero_dml'];
    case 'swap_parent_before_descendant_order':
      return ['parent_before_descendant_ascii_order', 'zero_dml'];
    case 'replace_exact_closure_key_with_alias':
      return ['closure_key_exact', 'zero_dml'];
    case 'add_unknown_bundle_or_entry_field':
      return ['bundle_closed_shape', 'entry_closed_shape', 'zero_dml'];
    case 'change_child_source_canonical_bytes':
      return ['source_hash_and_scope_binding', 'zero_dml'];
    case 'change_child_plan_without_rebinding_hash':
    ...
    case 'delete_persisted_child_generated_schema_binding_then_replay':
      return ['generated_schema_binding_set_exact', 'no_repair_on_replay'];
    case 'inject_before_first_t2a_write':
      return ['plan_table_unchanged', 'schema_binding_table_unchanged'];
    case 'inject_after_all_t2a_writes_before_commit':
      return [
    ...
    case 'submit_stale_run_or_scope_work_fence':
      return ['run_work_fence_exact', 'scope_work_fence_exact', 'zero_dml'];
    case 'reopen_then_detect_persisted_plan_or_schema_binding_tamper':
      return ['sqlite_reopen', 'tamper_detected', 'no_repair_on_recovery'];
    default:
      throw new Error(`Static child T2a behavior ${behavior} has no oracle`);
  }
}

function executeStaticChildT2aPositive(
  fixture: StaticChildPlanBundleFixtureCase,
  behavior: string,
  variants: readonly string[],
): StaticChildPlanBundleFixtureOracle {
  const context = staticChildT2aContext(fixture, variants.join('-'));
  const checks: string[] = [];
  if (
    behavior ===
    'persist_parent_unique_children_and_generated_schema_authorities'
  ) {
    requireBridge(
      canonicalJson([...variants]) === canonicalJson(['atomic_commit']),
      `${fixture.case_id} variants drifted`,
    );
    const result = persistCompileResultT2a(
      context.instance.store,
      context.input,
    );
    requireBridge(result.disposition === 'compiled', 'T2a did not compile');
    const expectedPlans = new Map<
      string,
      { source: JsonObject; plan: JsonObject }
    >([
      [
        context.compiledFixture.parentPlan.plan_hash,
        {
          source: G5_TEST_SOURCE,
          plan: context.compiledFixture.parentPlan,
        },
      ],
      ...context.compiledFixture.bundle.entries.map(
        (entry) =>
          [
            entry.plan.plan_hash,
            { source: entry.source, plan: entry.plan },
          ] as const,
      ),
    ]);
    const planRows = context.instance.store.queryAll<{
      plan_hash: string;
      source_json: string;
      compiled_plan_json: string;
    }>(
      `SELECT plan_hash, source_json, compiled_plan_json
         FROM workflow_graph_scope_plans WHERE graph_run_id = ?
         ORDER BY plan_hash COLLATE BINARY`,
      [context.created.activation.graphRunId],
    );
    requireBridge(
      planRows.length === expectedPlans.size && expectedPlans.size === 4,
      'T2a did not atomically persist parent plus unique child Plans',
    );
    checks.push('parent_and_unique_child_plans_atomic');
    const bindingCounts = context.instance.store.queryAll<{
      plan_hash: string;
      count: number;
    }>(
      `SELECT plan_hash, count(*) AS count
         FROM workflow_plan_generated_schemas WHERE graph_run_id = ?
         GROUP BY plan_hash ORDER BY plan_hash COLLATE BINARY`,
      [context.created.activation.graphRunId],
    );
    requireBridge(
      bindingCounts.length === expectedPlans.size &&
        bindingCounts.every((row) => row.count > 0),
      'T2a omitted generated-schema authority for a unique Plan',
    );
    checks.push('generated_schema_bindings_per_unique_plan');
    requireBridge(
      planRows.every((row) => {
        const expected = expectedPlans.get(row.plan_hash);
        return (
          expected !== undefined &&
          row.source_json === canonicalJson(expected.source) &&
          row.compiled_plan_json === canonicalJson(expected.plan)
        );
      }),
      'T2a persisted non-canonical source or Plan bytes',
    );
    checks.push('canonical_source_and_plan_bytes');
    requireBridge(
      context.compiledFixture.bundle.entries.length === 4 &&
        new Set(
          context.compiledFixture.bundle.entries.map(
            (entry) => entry.plan.plan_hash,
          ),
        ).size === 3,
      'T2a did not deduplicate shared child Plan content',
    );
    checks.push('shared_child_content_deduplicated');
    return {
  ...
  if (behavior === 'lose_commit_response_reopen_and_exact_replay') {
    requireBridge(
      canonicalJson([...variants]) ===
        canonicalJson([
          'response_loss',
          'same_connection_replay',
          'reopen_replay',
        ]),
      `${fixture.case_id} variants drifted`,
    );
    const lostResponse = persistCompileResultT2a(
      context.instance.store,
      context.input,
    );
    requireBridge(
      lostResponse.disposition === 'compiled',
      'T2a response-loss setup did not commit',
    );
    const committedState = staticChildPlanAndBindingState(context);
    checks.push('response_loss_recovery');
    const beforeSameConnectionChanges = staticChildTotalChanges(
      context.instance,
    );
    const sameConnectionReplay = persistCompileResultT2a(
      context.instance.store,
      context.input,
    );
    requireBridge(
      sameConnectionReplay.disposition === 'exact_replay' &&
        sameConnectionReplay.planId === lostResponse.planId &&
        staticChildTotalChanges(context.instance) ===
          beforeSameConnectionChanges,
      'same-connection replay was not literal zero-DML',
    );
    checks.push('same_connection_zero_dml');
    context.instance.closeStore();
    context.instance.reopenStore();
    const beforeReopenChanges = staticChildTotalChanges(context.instance);
    const reopenReplay = persistCompileResultT2a(
      context.instance.store,
      context.input,
    );
    requireBridge(
      reopenReplay.disposition === 'exact_replay' &&
        reopenReplay.planId === lostResponse.planId &&
        staticChildTotalChanges(context.instance) === beforeReopenChanges,
      'reopen replay was not literal zero-DML',
    );
    checks.push('reopen_zero_dml');
    requireBridge(
      staticChildPlanAndBindingState(context) === committedState,
      'canonical database state drifted across response loss and reopen',
    );
    checks.push('canonical_database_state_exact');
    return {
  ...
  throw new Error(`${fixture.case_id} positive T2a behavior is unhandled`);
}

function executeStaticChildT2aNegative(
  fixture: StaticChildPlanBundleFixtureCase,
  behavior: string,
  variants: readonly string[],
): StaticChildPlanBundleFixtureOracle {
  const checks = observedStaticChildChecks(behavior);
  let observedError: StaticChildPlanBundleFixtureOracle['exact_error'] = null;
  for (const variant of variants) {
    const context = staticChildT2aContext(fixture, variant);
    let input: Parameters<typeof persistCompileResultT2a>[1] = context.input;
    if (behavior === 'remove_last_bundle_member') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
          entries: context.compiledFixture.bundle.entries.slice(0, -1),
        },
      };
    } else if (behavior === 'append_excess_bundle_member') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
          entries: [
            ...context.compiledFixture.bundle.entries,
            context.compiledFixture.bundle.entries[0]!,
          ],
        },
      };
    } else if (behavior === 'replace_member_with_duplicate_closure_entry') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
          entries: context.compiledFixture.bundle.entries.map((entry, index) =>
            index === 3 ? context.compiledFixture.bundle.entries[1]! : entry,
          ),
        },
      };
    } else if (behavior === 'swap_parent_before_descendant_order') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
          entries: [
            context.compiledFixture.bundle.entries[1]!,
            context.compiledFixture.bundle.entries[0]!,
            ...context.compiledFixture.bundle.entries.slice(2),
          ],
        },
      };
    } else if (behavior === 'replace_exact_closure_key_with_alias') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
    ...
    } else if (behavior === 'add_unknown_bundle_or_entry_field') {
      input = {
        ...input,
        staticChildPlanBundle:
          variant === 'bundle_field'
            ? ({
                ...context.compiledFixture.bundle,
                unknown: true,
            ...
            : {
                ...context.compiledFixture.bundle,
                entries: context.compiledFixture.bundle.entries.map(
                  (entry, index) =>
                    index === 0
                      ? ({
                          ...entry,
    ...
    } else if (behavior === 'change_child_source_canonical_bytes') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
    ...
    } else if (behavior === 'change_child_plan_without_rebinding_hash') {
      input = {
        ...input,
        staticChildPlanBundle: {
          ...context.compiledFixture.bundle,
          entries: context.compiledFixture.bundle.entries.map((entry, index) =>
            index === 0
              ? {
                  ...
                  plan: {
                    ...entry.plan,
                    capability_catalog_hash: hash('bridge-plan-tamper'),
                  },
                }
              : entry,
          ),
      ...
      const { plan_hash: _planHash, ...withoutHash } = nested;
      void _planHash;
      const drift = replaceStaticChildPlan(
        context,
        0,
        withPlanHash({
          ...withoutHash,
      ...
      const initial = persistCompileResultT2a(context.instance.store, input);
      requireBridge(
        initial.disposition === 'compiled',
        `${fixture.case_id} persistence setup failed`,
      );
      const childHash = context.compiledFixture.uniqueChildPlanHashes[0]!;
      context.instance.store.withImmediateTransaction((transaction) => {
        if (behavior === 'tamper_persisted_child_plan_bytes_then_replay') {
          transaction.execute(
            "UPDATE workflow_graph_scope_plans SET compiled_plan_json = '{}' WHERE graph_run_id = ? AND plan_hash = ?",
            [context.created.activation.graphRunId, childHash],
          );
        } else {
          transaction.execute(
      ...
      throw new Error(`${fixture.case_id} negative T2a behavior is unhandled`);
    }
    const beforeState = staticChildPlanAndBindingState(context);
    const beforeChanges = staticChildTotalChanges(context.instance);
    const error = observeStaticChildError(() =>
      persistCompileResultT2a(context.instance.store, input),
    );
    requireStaticChildNoWrite(context, beforeState, beforeChanges);
    if (observedError === null) observedError = error;
    requireBridge(
      error === observedError,
      `${fixture.case_id} variants produced different errors`,
    );
  }
  return {
    disposition: 'rejected',
    sqlite_state: 'unchanged',
    exact_error: observedError,
    checks,
  };
}

function executeStaticChildT2aFault(
  fixture: StaticChildPlanBundleFixtureCase,
  behavior: string,
  variants: readonly string[],
): StaticChildPlanBundleFixtureOracle {
  const point = staticChildFaultPoint(fixture);
  const checks = observedStaticChildChecks(behavior);
  let observedError: StaticChildPlanBundleFixtureOracle['exact_error'] = null;
  for (const variant of variants) {
    const context = staticChildT2aContext(fixture, variant);
    ...
    let injectedFault:
      ...
      | undefined;
    if (behavior === 'inject_before_first_t2a_write') {
      requireBridge(
        point === 'before_first_write' && variant === point,
        `${fixture.case_id} fault binding drifted`,
      );
      injectedFault = { point: 'before_first_write' };
    } else if (behavior === 'inject_after_all_t2a_writes_before_commit') {
      requireBridge(
        point === 'before_commit' && variant === point,
        `${fixture.case_id} fault binding drifted`,
      );
      injectedFault = { point: 'before_commit' };
    } else if (behavior === 'submit_stale_build_row_version') {
      requireBridge(
        point === 'stale_build_row_version' && variant === 'row_version',
        `${fixture.case_id} stale row binding drifted`,
      );
      input = { ...input, expectedBuildRowVersion: 0 };
    } else if (behavior === 'submit_stale_compile_lease_identity_or_expiry') {
      requireBridge(
        point === 'stale_compile_lease' &&
          ['owner', 'token', 'expiry'].includes(variant),
        `${fixture.case_id} lease variant drifted`,
      );
      const expiresAtMs = variant === 'expiry' ? input.nowMs : 100;
      context.instance.store.withImmediateTransaction((transaction) => {
        transaction.execute(
          `UPDATE workflow_graph_scope_builds
            ...
            WHERE id = ?`,
          [
            'bridge-compiler-worker',
            'bridge-compile-token',
            expiresAtMs,
            context.created.activation.rootBuildId,
          ],
        );
      });
      input = {
    ...
    } else if (behavior === 'submit_stale_run_or_scope_work_fence') {
      requireBridge(
        point === 'stale_work_fence' &&
          ['run_fence', 'scope_fence'].includes(variant),
        `${fixture.case_id} work-fence variant drifted`,
      );
      context.instance.store.withImmediateTransaction((transaction) => {
        transaction.execute(
          variant === 'run_fence'
          ...
          [
            variant === 'run_fence'
              ? context.created.activation.graphRunId
              : context.created.activation.rootScopeId,
          ],
        );
      });
    } else if (
      ...
      requireBridge(
        point === 'reopen_tamper_recovery' &&
          ['compiled_plan_json', 'generated_schema_binding'].includes(variant),
        `${fixture.case_id} reopen recovery variant drifted`,
      );
      const initial = persistCompileResultT2a(context.instance.store, input);
      requireBridge(
        initial.disposition === 'compiled',
      ...
      throw new Error(`${fixture.case_id} fault T2a behavior is unhandled`);
    }
    const beforeState = staticChildPlanAndBindingState(context);
    const beforeChanges = staticChildTotalChanges(context.instance);
    const error = observeStaticChildError(() =>
      persistCompileResultT2a(context.instance.store, input, injectedFault),
    );
    requireBridge(
      staticChildPlanAndBindingState(context) === beforeState,
      `${fixture.case_id} rejection or rollback changed durable state`,
    );
    if (injectedFault === undefined) {
      requireBridge(
        staticChildTotalChanges(context.instance) === beforeChanges,
        `${fixture.case_id} stale/recovery rejection executed DML`,
      );
    }
    if (observedError === null) observedError = error;
    requireBridge(
...
function executeStaticChildT2aFixture(
  fixture: StaticChildPlanBundleFixtureCase,
): StaticChildPlanBundleFixtureOracle {
  const { behavior, variants } = staticChildBridgeBehavior(fixture);
  switch (fixture.category) {
    case 'positive':
      return executeStaticChildT2aPositive(fixture, behavior, variants);
    case 'negative':
      return executeStaticChildT2aNegative(fixture, behavior, variants);
    case 'fault':
      return executeStaticChildT2aFault(fixture, behavior, variants);
  }
}
...
function readStaticChildBridgeCases(name: string): JsonValue[] {
  const artifact = JSON.parse(
    fs.readFileSync(
      new URL(
        `../contracts/conformance/static-child-plan-bundle-repair/${name}`,
        import.meta.url,
      ),
      'utf8',
    ),
...
const staticChildBridgeArtifacts: StaticChildPlanBundleFixtureArtifacts = {
  positive: readStaticChildBridgeCases('positive-cases.json'),
  negative: readStaticChildBridgeCases('negative-cases.json'),
  fault: readStaticChildBridgeCases('fault-cases.json'),
};
const staticChildBridgeHandlers: readonly StaticChildPlanBundleFixtureHandler[] =
  [
    {
      id: 'compile_workflow_production',
      execute: executeStaticChildCompilerFixture,
    },
    {
      id: 'persist_static_child_plan_bundle_t2a_production',
      execute: executeStaticChildT2aFixture,
    },
  ];
const staticChildBridgeHarness =
  new StaticChildPlanBundleFixtureExecutionHarness(
    staticChildBridgeArtifacts,
    staticChildBridgeHandlers,
  );
...
describe('focused static child Plan bridge execution gate', () => {
  ...
  it.each(staticChildBridgeHarness.fixtures)(
    ...
    (fixture) => {
      const receipt = staticChildBridgeHarness.execute(fixture);
