...
CREATE TABLE "workflow_registry_resources" (
  ...
  CONSTRAINT "ck:workflow_registry_resources:owner_core_ref:owner_feature_id:owner_principal_ref:exactly_one" CHECK ((("owner_core_ref" IS NOT NULL) + ("owner_feature_id" IS NOT NULL) + ("owner_principal_ref" IS NOT NULL)) = 1) /* check_kind=exactly_one logical_columns=owner_core_ref,owner_feature_id,owner_principal_ref */,
...
CREATE TABLE "workflow_runtime_command_ingress_invocations" (
  ...
  CONSTRAINT "ck:workflow_runtime_command_ingress_invocations:entrypoint:enum" CHECK ("entrypoint" IN ('runtime_center', 'feature_page', 'feature_host_api', 'external_api', 'automation', 'task_workspace', 'card_action', 'deadline_watchdog')) /* check_kind=enum_membership logical_columns=entrypoint */,
