...
} from '../contracts/g3-pack-release-activation-fixtures.js';
import {
  G39_EVENT_DOMAIN,
...
import {
  activatePackRelease,
  recoverPendingPackReleaseActivations,
  type PackReleaseActivationFaultPoint,
} from './pack-release-activation.js';
...
function reviewRegistry(
  fixture: ReturnType<typeof g39PackReleaseActivationStoreFixtureForTest>,
...
function bytes(request: G39PackReleaseActivationRequest): Buffer {
...
function activationCounts(store: WorkflowRuntimeStore) {
  return store.queryOne<Record<string, number>>(
    `SELECT
      (SELECT COUNT(*) FROM workflow_pack_release_activation_commands) AS commands,
      (SELECT COUNT(*) FROM workflow_pack_release_activation_invocations) AS invocations,
      (SELECT COUNT(*) FROM workflow_pack_release_activation_events) AS events,
      (SELECT COUNT(*) FROM workflow_pack_active_releases) AS pointers`,
    [],
  )!;
}
...
function installActivePrevious(
  ...
  const previousReleaseId = 'pack-release:fixture.previous@0.9.0';
  ...
  const previousRetention: G39RetentionClaim = {
    ...
    pack_release_id: previousReleaseId,
  ...
  return {
    ...
    releases: store.queryAll(
      `SELECT id, status, activated_at_ms, row_version
         FROM workflow_pack_releases ORDER BY id`,
      [],
    ),
    pointers: store.queryAll(
      `SELECT pack_id, release_id, release_hash, row_version, activated_at_ms
         FROM workflow_pack_active_releases ORDER BY pack_id`,
      [],
    ),
    activationValues: store.queryAll(
      `SELECT id, content_hash FROM workflow_values
        WHERE provenance_ref = 'icarus.pack-release-activation/1'
        ORDER BY id`,
      [],
    ),
  };
    ...
    transaction.execute(
      `INSERT INTO workflow_values (
        ...
        row_version
                ...
                ?, ?, 'icarus.pack-release-activation/1', 'pinned', 'live',
                NULL, ?, 1)`,
      [
        requestValueId,
        canonical,
...
describe('G3.9 Pack Release Activation', () => {
  ...
  it('applies an absent pointer once, returns the original receipt on exact replay, and always conflicts on drift', () => {
    ...
    const applied = activatePackRelease(
      store,
    ...
    expect(applied).toMatchObject({
      disposition: 'applied',
      code: 'pack_release_activation_applied',
      invocation_no: 1,
    ...
    expect(
      store.queryOne<{ release_id: string; row_version: number }>(
        `SELECT release_id, row_version FROM workflow_pack_active_releases
          WHERE pack_id = ?`,
        [fixture.activation_request.pack_id],
      ),
    ).toEqual({
    ...
    const duplicate = activatePackRelease(
      store,
      fixture.activation_request_bytes,
      {
        ...fixture.activation_invocation,
    ...
    expect(duplicate).toMatchObject({
      ...
      code: 'pack_release_activation_duplicate',
      invocation_no: 2,
    ...
    const firstConflict = activatePackRelease(store, bytes(drift), {
      ...fixture.activation_invocation,
      requested_at_ms: drift.requested_at_ms,
    });
    const repeatedConflict = activatePackRelease(store, bytes(drift), {
      ...fixture.activation_invocation,
    ...
    expect(
      store.queryOne<{ row_version: number }>(
        `SELECT row_version FROM workflow_pack_active_releases
          WHERE pack_id = ?`,
        [fixture.activation_request.pack_id],
      ),
  ...
  it('moves the previous active Release to draining and advances a present pointer by one', () => {
    ...
    const result = activatePackRelease(
      store,
    ...
    expect(
      store.queryAll<{ id: string; status: string }>(
        `SELECT id, status FROM workflow_pack_releases
  ...
  it.each([
    {
      ...
      prepare: (
        ...
        store.withImmediateTransaction((transaction) => {
          transaction.execute(
            `DELETE FROM workflow_pack_release_resources
              WHERE release_id = ? AND resource_id = (
                SELECT resource_id FROM workflow_pack_release_resources
    ...
    ({ expectedCode, prepare }) => {
      ...
      const replay = activatePackRelease(store, bytes(request), {
        ...fixture.activation_invocation,
        requested_at_ms: fixture.activation_invocation.requested_at_ms + 1,
      });
  ...
  it('terminalizes pointer CAS mismatch as conflict without Release or pointer mutation', () => {
    ...
    const replay = activatePackRelease(
      store,
      fixture.activation_request_bytes,
      {
        ...fixture.activation_invocation,
  ...
  ] satisfies PackReleaseActivationFaultPoint[])(
    ...
    expect(
      recoveredStore
        .queryAll<{ event_type: string }>(
          `SELECT event_type FROM workflow_pack_release_activation_events
  ...
  it('bounded recovery resumes a clean pending command from its canonical request Value', () => {
    ...
    const recovered = recoverPendingPackReleaseActivations(store, {
      limit: 1,
    ...
    expect(
      store.queryOne<{ lifecycle: string }>(
        `SELECT lifecycle FROM workflow_pack_release_activation_commands`,
        [],
      ),
  ...
  it('keeps pending same-key drift non-terminal, conflicts repeatedly, then recovers the bound request', () => {
    ...
    rehashG39ActivationRequest(drift);
    const first = activatePackRelease(store, bytes(drift), {
      ...fixture.activation_invocation,
      requested_at_ms: drift.requested_at_ms,
    });
    const second = activatePackRelease(store, bytes(drift), {
      ...fixture.activation_invocation,
      requested_at_ms: drift.requested_at_ms + 1,
    });
  ...
  it('fails closed on inconsistent transition evidence in a pending command', () => {
    ...
    expect(() =>
      recoverPendingPackReleaseActivations(store, {
        limit: 1,
        actor_ref: fixture.activation_request.actor_ref,
  ...
  it('keeps verified target facts immutable while an Activation is pending', () => {
    ...
    expect(() =>
      store.withImmediateTransaction((transaction) => {
        transaction.execute(
          `UPDATE workflow_pack_release_activation_commands
              SET verified_target_pack_release_ref = 'tampered',
                  row_version = 2
            WHERE lifecycle = 'pending'`,
          [],
        );
  ...
  it.each([
    ...
    },
    {
      ...
      tamper: (store: WorkflowRuntimeStore) => {
        ...
        rawTamper(
          store,
          `UPDATE workflow_pack_release_activation_invocations
    ...
    },
    {
      ...
      tamper: (store: WorkflowRuntimeStore) => {
        ...
        rawTamper(
          store,
          `UPDATE workflow_pack_release_activation_events SET event_hash = ?
            WHERE event_no = 1`,
          [
    ...
    },
    {
      ...
      tamper: (store: WorkflowRuntimeStore) => {
        ...
        rawTamper(
          store,
          `UPDATE workflow_pack_release_activation_events SET attempt_no = 99
            WHERE event_no = 1`,
        );
      },
    },
    ...
    tamper(store);
    expect(() =>
      activatePackRelease(store, fixture.activation_request_bytes, {
        ...fixture.activation_invocation,
        invocation_kind: 'recovery',
  ...
  it('fails closed when a schema-valid Event is appended with a recomputed hash', () => {
    ...
    >(
      `SELECT command_id, event_no, attempt_no, phase, event_type, failure_code,
              ...
              verified_previous_pack_release_hash, detail_value_id,
    ...
    rawTamper(
      ...
      [
        ...
        appended.verified_pack_id,
        appended.verified_target_pack_release_id,
        appended.verified_target_pack_release_ref,
        appended.verified_target_pack_release_version,
        appended.verified_target_pack_release_hash,
        appended.verified_previous_pack_release_id,
        appended.verified_previous_pack_release_ref,
        appended.verified_previous_pack_release_version,
        appended.verified_previous_pack_release_hash,
  ...
  it('preserves active Release deletion and held Retention release protections', () => {
    ...
    expect(() =>
      store.withImmediateTransaction((transaction) => {
        transaction.execute(`DELETE FROM workflow_pack_releases WHERE id = ?`, [
          fixture.activation_request.target_release.release_id,
        ]);
      }),
    ).toThrow('active_or_draining_pack_release_delete_forbidden');
    expect(() =>
      store.withImmediateTransaction((transaction) => {
        transaction.execute(
          `UPDATE workflow_registry_retention_handles
  ...
  it('produces identical semantic rows in two independent real-file databases', () => {
    ...
    const firstResult = activatePackRelease(
      firstStore,
    ...
    const secondResult = activatePackRelease(
      secondStore,
    ...
    const snapshot = (store: WorkflowRuntimeStore) => ({
      commands: store.queryAll(
        `SELECT * FROM workflow_pack_release_activation_commands ORDER BY command_id`,
        [],
      ),
      invocations: store.queryAll(
        `SELECT * FROM workflow_pack_release_activation_invocations
          ORDER BY command_id, invocation_no`,
        [],
      ),
      events: store.queryAll(
        `SELECT * FROM workflow_pack_release_activation_events
      ...
      pointers: store.queryAll(
        `SELECT * FROM workflow_pack_active_releases ORDER BY pack_id`,
        [],
      ),
    });
