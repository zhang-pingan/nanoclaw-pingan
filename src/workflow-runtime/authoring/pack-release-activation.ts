...
export type PackReleaseActivationFaultPoint =
  | 'after_request_value'
...
export interface PackReleaseActivationOptions {
  readonly faultInjector?: (point: PackReleaseActivationFaultPoint) => void;
}

export interface PackReleaseActivationRecoveryOptions {
...
export class PackReleaseActivationError extends Error {
  constructor(
    ...
    this.name = 'PackReleaseActivationError';
  }
}
...
interface CommandRow extends Record<string, unknown> {
  ...
  verified_pack_id: string | null;
  verified_target_pack_release_id: string | null;
  verified_target_pack_release_ref: string | null;
  verified_target_pack_release_version: string | null;
  verified_target_pack_release_hash: string | null;
  verified_previous_pack_release_id: string | null;
  verified_previous_pack_release_ref: string | null;
  verified_previous_pack_release_version: string | null;
  verified_previous_pack_release_hash: string | null;
  verified_target_retention_handle_id: string | null;
  verified_target_retention_handle_kind: string | null;
  verified_target_retention_pack_release_id: string | null;
  ...
  verified_previous_retention_pack_release_id: string | null;
  ...
  observed_pack_release_id: string | null;
  observed_pack_release_ref: string | null;
  observed_pack_release_version: string | null;
  observed_pack_release_hash: string | null;
...
interface ActivationEventInput {
  ...
  event_type:
    ...
    | 'integrity_failed';
  failure_code: string | null;
  detail: G39PackReleaseActivationResult | null;
  includeVerifiedReleaseFacts: boolean;
}

const PROVENANCE_REF = 'icarus.pack-release-activation/1';
...
const COMMAND_COLUMNS = `command_id, idempotency_domain, idempotency_key,
  ...
  verified_pack_id,
  verified_target_pack_release_id, verified_target_pack_release_ref,
  verified_target_pack_release_version, verified_target_pack_release_hash,
  verified_previous_pack_release_id, verified_previous_pack_release_ref,
  verified_previous_pack_release_version,
  verified_previous_pack_release_hash, verified_target_retention_handle_id,
  verified_target_retention_handle_kind,
  verified_target_retention_pack_release_id,
  ...
  verified_previous_retention_pack_release_id,
  ...
  observed_pointer_row_version, observed_pack_release_id,
  observed_pack_release_ref, observed_pack_release_version,
  observed_pack_release_hash, terminal_disposition,
...
function assertInvocation(invocation: G39ActivationInvocation): void {
  if (
    ...
    throw new PackReleaseActivationError(
...
export function parseG39ActivationRequestBytes(
  ...
  try {
    validateG39PackReleaseActivationRequest(parsed);
  } catch (error) {
    const code =
...
function loadCommand(
  ...
  return connection.queryOne<CommandRow>(
    `SELECT ${COMMAND_COLUMNS}
       FROM workflow_pack_release_activation_commands
...
function loadRetention(
  ...
  return transaction.queryOne<RetentionRow>(
    `SELECT id, handle_kind, pack_release_id, closure_manifest_id,
...
function loadPointer(
  ...
  return transaction.queryOne<PointerRow>(
    `SELECT pointer.pack_id, pointer.release_id, pointer.release_hash,
            pointer.row_version, release.release_ref, release.release_version
       FROM workflow_pack_active_releases AS pointer
       JOIN workflow_pack_releases AS release ON release.id = pointer.release_id
      WHERE pointer.pack_id = ?`,
    [packId],
  );
}
...
function exactReleaseResources(
  ...
  const rows = transaction.queryAll<{
    ...
    `SELECT resource.resource_type, resource.resource_id,
       ...
       FROM workflow_pack_release_resources AS member
    ...
    `SELECT resource.resource_type, value.inline_canonical_json
      ...
      WHERE member.release_id = ?
        AND resource.resource_type IN (
          'pack_execution_artifact', 'executor_implementation'
        )
      ORDER BY resource.resource_type, resource.resource_id,
               resource.resource_version`,
    [releaseId],
...
function retentionIdentityMatches(
  ...
  return (
    ...
    row.pack_release_id === claim.pack_release_id &&
...
function insertPendingCommand(
  ...
  transaction.execute(
    `INSERT INTO workflow_pack_release_activation_commands (
      ...
      row_version
    ) VALUES (?, 'activate_pack_release', ?, ?, ?, ?, ?, ?, ?, 'pending', ?, 0)`,
    [
      commandId,
...
function verifiedUpdate(
  ...
  if (facts.target) {
    set('verified_pack_id', facts.target.pack_id);
    set('verified_target_pack_release_id', facts.target.id);
    set('verified_target_pack_release_ref', facts.target.release_ref);
    set('verified_target_pack_release_version', facts.target.release_version);
    set('verified_target_pack_release_hash', facts.target.release_hash);
  }
  if (facts.previous) {
    set('verified_previous_pack_release_id', facts.previous.id);
    set('verified_previous_pack_release_ref', facts.previous.release_ref);
    set(
      'verified_previous_pack_release_version',
      facts.previous.release_version,
    );
    set('verified_previous_pack_release_hash', facts.previous.release_hash);
  }
  const retention = (
    ...
    if (identity) {
      ...
      set(
        `verified_${prefix}_retention_pack_release_id`,
        identity.pack_release_id,
      );
      set(
  ...
  if (facts.pointer) {
    ...
    if (facts.pointer.state === 'present') {
      ...
      set('observed_pack_release_id', facts.pointer.release.release_id);
      set('observed_pack_release_ref', facts.pointer.release.ref.id);
      set('observed_pack_release_version', facts.pointer.release.ref.version);
      set('observed_pack_release_hash', facts.pointer.release.hash);
    }
  }
...
function persistVerifiedFacts(
  ...
  const result = transaction.execute(
    `UPDATE workflow_pack_release_activation_commands
  ...
  return transaction.queryOne<CommandRow>(
    `SELECT ${COMMAND_COLUMNS} FROM workflow_pack_release_activation_commands WHERE command_id = ?`,
    [command.command_id],
  )!;
}
...
function nextInvocation(
  ...
  const head = transaction.queryOne<{
    ...
    `SELECT invocation_no, invocation_hash
       FROM workflow_pack_release_activation_invocations
...
function insertInvocation(
  ...
  transaction.execute(
    `INSERT INTO workflow_pack_release_activation_invocations (
...
function insertEvents(
  ...
  const head = transaction.queryOne<{ event_no: number; event_hash: string }>(
    `SELECT event_no, event_hash
       FROM workflow_pack_release_activation_events
  ...
  for (const input of inputs) {
    const target = input.includeVerifiedReleaseFacts
      ? {
          pack: command.verified_pack_id,
          id: command.verified_target_pack_release_id,
          ref: command.verified_target_pack_release_ref,
          version: command.verified_target_pack_release_version,
          hash: command.verified_target_pack_release_hash,
        }
      : null;
    const previous = input.includeVerifiedReleaseFacts
      ? {
          id: command.verified_previous_pack_release_id,
          ref: command.verified_previous_pack_release_ref,
          version: command.verified_previous_pack_release_version,
          hash: command.verified_previous_pack_release_hash,
        }
      : null;
    ...
    const rowWithoutHash: Omit<EventRow, 'event_hash'> = {
      ...
      verified_pack_id: target?.pack ?? null,
      verified_target_pack_release_id: target?.id ?? null,
      verified_target_pack_release_ref: target?.ref ?? null,
      verified_target_pack_release_version: target?.version ?? null,
      verified_target_pack_release_hash: target?.hash ?? null,
      verified_previous_pack_release_id: previous?.id ?? null,
      verified_previous_pack_release_ref: previous?.ref ?? null,
      verified_previous_pack_release_version: previous?.version ?? null,
      verified_previous_pack_release_hash: previous?.hash ?? null,
    ...
    transaction.execute(
      `INSERT INTO workflow_pack_release_activation_events (
        command_id, event_no, attempt_no, phase, event_type, failure_code,
        verified_pack_id, verified_target_pack_release_id,
        verified_target_pack_release_ref,
        verified_target_pack_release_version,
        verified_target_pack_release_hash,
        verified_previous_pack_release_id,
        verified_previous_pack_release_ref,
        verified_previous_pack_release_version,
        verified_previous_pack_release_hash, detail_value_id, detail_hash,
...
function samePointerState(
  ...
  return pointerMatches(
    request,
    observedPointer(loadPointer(transaction, request.pack_id)),
  );
}
...
function verifyInvocationAndEventChains(
  ...
  const invocationResults: G39PackReleaseActivationResult[] = [];
  for (const [index, row] of invocations.entries()) {
    ...
    validateG39PackReleaseActivationResult(value.parsed);
    const result = value.parsed as G39PackReleaseActivationResult;
  ...
  for (const [index, row] of events.entries()) {
    ...
    if (row.detail_value_id) {
      ...
      validateG39PackReleaseActivationResult(detail.parsed);
      const detailResult = detail.parsed as G39PackReleaseActivationResult;
      if (
    ...
    if (
      ...
      attemptEvents.some((row, eventIndex) => {
        ...
        return (
          ...
          row.verified_pack_id !==
            (withVerifiedFacts ? command.verified_pack_id : null) ||
          row.verified_target_pack_release_id !==
            (withVerifiedFacts
              ? command.verified_target_pack_release_id
              : null) ||
          row.verified_target_pack_release_ref !==
            (withVerifiedFacts
              ? command.verified_target_pack_release_ref
              : null) ||
          row.verified_target_pack_release_version !==
            (withVerifiedFacts
              ? command.verified_target_pack_release_version
              : null) ||
          row.verified_target_pack_release_hash !==
            (withVerifiedFacts
              ? command.verified_target_pack_release_hash
              : null) ||
          row.verified_previous_pack_release_id !==
            (withVerifiedFacts
              ? command.verified_previous_pack_release_id
              : null) ||
          row.verified_previous_pack_release_ref !==
            (withVerifiedFacts
              ? command.verified_previous_pack_release_ref
              : null) ||
          row.verified_previous_pack_release_version !==
            (withVerifiedFacts
              ? command.verified_previous_pack_release_version
              : null) ||
          row.verified_previous_pack_release_hash !==
            (withVerifiedFacts
              ? command.verified_previous_pack_release_hash
              : null) ||
          row.detail_value_id !==
...
function verifyExistingCommand(
  ...
  try {
    validateG39PackReleaseActivationRequest(requestValue.parsed);
  } catch (error) {
    throw new PackReleaseActivationError(
      'terminal_integrity_mismatch',
  ...
  const request = requestValue.parsed as G39PackReleaseActivationRequest;
  if (
    command.command_id !==
      g39ActivationCommandId(
        request.idempotency_domain,
  ...
  if (
    !compareFact(command.verified_pack_id, request.pack_id) ||
    !compareFact(
      command.verified_target_pack_release_id,
      request.target_release.release_id,
    ) ||
    ...
    !compareFact(
      command.verified_target_retention_pack_release_id,
    ...
    !compareFact(
      command.verified_previous_retention_pack_release_id,
      request.previous_retention?.pack_release_id ?? null,
    ) ||
    !compareFact(
  ...
  validateG39PackReleaseActivationResult(resultValue.parsed);
  const terminalResult = resultValue.parsed as G39PackReleaseActivationResult;
    ...
    command.observed_pointer_state === null
      ...
      : command.observed_pointer_state === 'absent'
        ...
        : {
            ...
            release: {
              release_id: command.observed_pack_release_id!,
              ref: {
                id: command.observed_pack_release_ref!,
                version: command.observed_pack_release_version!,
              },
              hash: command.observed_pack_release_hash as Sha256Hash,
            },
          };
  if (
  ...
  if (command.lifecycle === 'applied') {
    ...
    validateG39PackReleaseActivationReceipt(receiptValue.parsed);
    const receipt = receiptValue.parsed as G39PackReleaseActivationReceipt;
    if (
      command.canonical_receipt_value_id !==
      ...
      receipt.pack_id !== request.pack_id ||
...
function finalizeCommand(
  ...
  result: G39PackReleaseActivationResult,
  request: G39PackReleaseActivationRequest,
  receipt: G39PackReleaseActivationReceipt | null,
...
function writeTerminal(
  ...
  failure: G39PackReleaseActivationResult['failure'],
  receipt: G39PackReleaseActivationReceipt | null,
  appliedPointerRowVersion: number | null,
  options: PackReleaseActivationOptions,
): G39PackReleaseActivationResult {
  const next = nextInvocation(transaction, command.command_id);
  const result = buildG39Result({
    format: 'icarus.workflow-pack-release-activation-result/1',
    disposition,
    code:
      disposition === 'applied'
...
function appendReplayOrDrift(
  transaction: WorkflowRuntimeWriteTransaction,
  command: CommandRow,
  boundRequest: G39PackReleaseActivationRequest,
  submittedRequest: G39PackReleaseActivationRequest,
  invocation: G39ActivationInvocation,
): G39PackReleaseActivationResult {
  const drift =
  ...
  const next = nextInvocation(transaction, command.command_id);
  let receipt: G39PackReleaseActivationReceipt | null = null;
  if (!drift && command.lifecycle === 'applied') {
    ...
    validateG39PackReleaseActivationReceipt(value.parsed);
    receipt = value.parsed as G39PackReleaseActivationReceipt;
  }
  const result = buildG39Result({
    format: 'icarus.workflow-pack-release-activation-result/1',
    disposition: drift ? 'conflict' : 'duplicate',
    code: drift ? 'idempotency_conflict' : 'pack_release_activation_duplicate',
    command_id: command.command_id,
    invocation_no: next.invocationNo,
...
function executeFirstOrPending(
  ...
  if (target.pack_id !== request.pack_id)
    return admittedFailure(
      transaction,
      command,
      request,
  ...
  if (request.previous_release) {
    ...
    if (previous.pack_id !== request.pack_id)
      return admittedFailure(
        transaction,
        command,
        request,
  ...
  facts.pointer = observedPointer(loadPointer(transaction, request.pack_id));
  ...
  if (request.previous_release) {
    const transition = transaction.execute(
      `UPDATE workflow_pack_releases
        ...
        WHERE id = ? AND pack_id = ? AND release_hash = ?
  ...
  const receipt = buildG39Receipt({
    format: 'icarus.workflow-pack-release-activation-receipt/1',
...
export function activatePackRelease(
  ...
  let result: G39PackReleaseActivationResult;
  try {
    ...
    if (error instanceof PackReleaseActivationError) throw error;
    if (
...
export function recoverPendingPackReleaseActivations(
  store: WorkflowRuntimeStore,
  options: PackReleaseActivationRecoveryOptions,
): G39PackReleaseActivationResult[] {
  if (
