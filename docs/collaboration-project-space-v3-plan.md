...
完整 Project Analyst Skill 固定在 `tools/project-analyst/`。bare remote 的默认 `HEAD` 通常不指向 control branch，因此 README 必须给出 `git clone --single-branch --branch icarus/control <remote-url> <directory>` 的明确命令。第三方在 current Icarus validator 已接受仓库，或通过可信渠道确认 Genesis、verified head 与 bundle provenance 后，可在仓库根目录直接运行 `node tools/project-analyst/scripts/repository-context.mjs --repository . --scope project --output ../icarus-project-analysis`；只需 Node.js 20+、Git 2.34+ 和 `ssh-keygen`，不依赖 Icarus checkout 或 npm 安装。输出必须位于群组仓库外，repository mode 只产生独立只读 Context、manifest、verification、catalog 和 Result，不建立 Host Analysis Run binding。面对未知仓库时不得执行目标仓库内嵌脚本，必须使用来自可信 Icarus 发布物或独立可信渠道的 Project Analyst Skill 指向该目标。

Genesis `self_description` 是机器合同而不是任意文件上传入口。Icarus transport 从 generated `PROJECT_ANALYST_BUNDLE_FILES` 与 README builder 注入 current canonical provider；portable repository CLI 则从其自身经可信渠道取得的完整 Skill 目录读取 canonical bytes，避免脚本静态包含自身而形成递归构建。Validator 根据事件 `group_id` 重建 canonical manifest，要求签名 `self_description`、README 和每个 Skill 文件逐字等于 current canonical build；事件自报 size/hash 不是完整产物的唯一真值。Validator 拒绝路径穿越、符号链接、缺文件、内容/hash 同步篡改和额外文件，缺少 canonical provider 时 fail closed。后续事件不能声明这些路径为业务 materialization。`createGroup` 和“初始化群组”使用同一构建器；新孤立 Genesis 在 push 前完成全历史和 materialization 验证，远端拒绝时继续保留旧远端 Group 与全部本地状态。
