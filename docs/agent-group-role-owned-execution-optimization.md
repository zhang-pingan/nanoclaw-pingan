...
3. 固定 Implementation/Action/Prompt/Handoff hash。
4. 根据 mode 创建 `PENDING_START` Turn。
5. 推送签名 `turn_created` 事件。
...
`action_completed` 只表示 Provider 已返回结果，不推进 FSM。角色用户确认后提交 `turn_completion_confirmed`，随后由同一协议命令生成 `turn_completed`。
...
- Role 和人数；
- 创建者初始 Role；
- State、owner_role、Outcome 和 target State。
