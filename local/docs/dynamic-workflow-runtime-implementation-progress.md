...
| SQLite/native/release observation | Store `certification_observation` mode、real-file Schema 11 Connection Factory | Production PRAGMA回验、active managed installation、better-sqlite3 `12.11.1`/native/SQLite/source/compile-options/Core/Launcher全非null | 待实现 |
| minimum machine class | versioned minimum-machine contract与observation | Apple Silicon M2+、16 GiB、APFS internal SSD、AC power、release/no interference closed result | 待实现 |
| startup smoke | release certification entry + bounded isolated DATA_DIR/STORE_DIR harness | 同数据卷临时DB、Schema 11/Production PRAGMA、duration/hash、不得触碰Runtime DB | 待实现 |
