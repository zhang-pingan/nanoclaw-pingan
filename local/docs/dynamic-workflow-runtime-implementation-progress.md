...
| G7 Control / Card / Projection / Recovery | `IN_PROGRESS/NOT_DONE` | G6 | replacement candidate新增closed immutable pre-resolution authenticated ingress audit，并修复exact replay、strict confirmation TTL/intent、T6e persisted source binding与authoritative projection rebuild；完整内部affected-chain已通过，等待controller执行一次fresh independent whole-Gate acceptance | `4b68940` + `7ec7aee` + rejected `db78b42` + 本原子repair提交 |
