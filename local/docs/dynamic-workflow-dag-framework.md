...
该authority是新的`current_construction_gate_ownership`治理身份，只声明未来G5/G7施工/fixture ownership，不改写G0.4/G1历史artifact，不进入G4 pack或Runtime运行时authority，也不授权任何G5业务实现。当前状态为`G5_OWNERSHIP_EXIT_CANDIDATE_PENDING_INDEPENDENT_REGRESSION`；下一独立ownership/affected-chain regression通过前，G5 Basic Runtime保持`NOT_READY`。
