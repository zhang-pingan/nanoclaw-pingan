...
G4当前状态为`EXIT_CANDIDATE_PENDING_INDEPENDENT_G4_REGRESSION`。本切片只建立后续G5可显式消费的isolated Store/fixture/Fake Adapter/Virtual Clock环境，不创建Workflow/Run/Activation或写任何Runtime业务表；没有T0-T8、Reconciler、Scheduler、Ledger、Wait/Inbox/Outbox、Runtime Command、Projection、真实Adapter、Feature/API/Automation ingress、Production loader/launchability、Execution Artifact build/install、Retention/Blob GC/delete或legacy fallback/compatibility reader。下一独立任务固定为G4 whole-gate regression；其通过前G5保持`NOT_READY`。
