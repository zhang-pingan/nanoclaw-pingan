...
> **状态**: 已确认目标架构、部署边界、创建溯源、恢复协议、首发 Safety/Capacity/SQLite Profile、精确工具链、Capacity Admin 控制面、Command/Golden 治理与开发期 absence baseline（Store 开工受 Capacity Control-Plane Contract Addendum 与 Executable DDL Gate 约束，Production Runtime 激活受 exact release readiness evidence、静态 absence baseline 与产品 surface coverage 门禁约束）
> **范围**: Icarus core workflow runtime
> **目标**: 统一静态 workflow、并行执行、运行时 DAG、条件路由、局部汇合、持久等待、子图和受约束动态扩图。
> **未来独立方案**: `local/docs/dynamic-workflow-runtime-extended-certification-plan.md` 仅在用户明确决定时执行，不属于 G8、G9、默认 CI、release startup 或 automation 依赖。
...
- [Capacity 管理、发布与审计](#capacity-管理发布与审计)
...
5. 每个工作包完成时，按其联读章节定位 [测试策略与模型验证](#测试策略与模型验证) 和 [完整验收标准](#完整验收标准) 中的对应门禁；Store 开工受 [Executable DDL Gate](#executable-ddl-gate) 约束，Production Runtime 激活另受 exact content-addressed release readiness evidence、`local_single_user` deployment profile、开发期静态 absence baseline 与 `ProductSurfaceCoverageManifest` 约束。
...
| I4 | Runtime Store、SQLite typed relation、Value/Blob 与 executable migration | [持久化字段、时间与 SQLite 约束](#持久化字段时间与-sqlite-约束)、[持久化模型](#持久化模型) | [Capacity 管理、发布与审计](#capacity-管理发布与审计)、[SQLite 关系展开规则](#sqlite-关系展开规则)、[Immutable Value/Blob Store](#immutable-valueblob-store)、[事务边界与 CAS](#事务边界与-cas)、[SQLite Execution Profile](#sqlite-execution-profile)、[Snapshot 与 Checkpoint](#snapshot-与-checkpoint) | Capacity Admin Logical Schema、Executable DDL Gate、Schema Manifest、schema-lint/constraint/query-plan fixtures |
| I5 | Graph 状态机、fixed-point reconcile、Scheduler、Ledger 与G5 Operational Blocker create/open/cache invariant | [Graph 与 Node 状态模型](#graph-与-node-状态模型)、[Resource Ledger 与调度](#resource-ledger-与调度) | [Capacity 管理、发布与审计](#capacity-管理发布与审计)、[Control Edge、Condition 与 Trigger](#control-edgecondition-与-trigger)、[Data Edge、Port 与 Input Seal](#data-edgeport-与-input-seal)、[Completion Policy、Early Close 与 Named Exit](#completion-policyearly-close-与-named-exit)、[事务边界与 CAS](#事务边界与-cas) | Capacity revision/change lineage、T2b、T3a、T3b、T4、Operational Blocker create/open/cache、Supported Limits；不含T6e resolution/abandon |
...
| I9 | Completion、Pause/Cancel、Compensation、Operational Blocker、Root Finalization、Root Coordinator 与 Recovery | [Completion Policy、Early Close 与 Named Exit](#completion-policyearly-close-与-named-exit)、[Retry、Pause、Cancel 与 Compensation](#retrypausecancel-与-compensation) | [Edge Resolution、Candidate 与 Cut](#edge-resolutioncandidate-与-cut)、[Workflow 与 Run](#workflow-与-run)、[事务边界与 CAS](#事务边界与-cas)、[Outbox、Lease 与恢复](#outboxlease-与恢复)、[Snapshot 与 Checkpoint](#snapshot-与-checkpoint) | G5只拥有blocker create/open/cache；G6拥有T7a/T7b/T8 close；G7拥有T6e、blocker resolution/abandon、last-blocker restoration、Recovery与相关Fault fixtures |
| I10 | Runtime Command、Capacity Admin、权限、Runtime Center 与 Trace | [Workflow Runtime Command 授权与审计](#workflow-runtime-command-授权与审计)、[Capacity 管理、发布与审计](#capacity-管理发布与审计)、[Runtime Center（运行中心）与 Trace](#runtime-center运行中心与-trace) | [权限与安全](#权限与安全)、[SQLite Execution Profile](#sqlite-execution-profile)、[Outbox、Lease 与恢复](#outboxlease-与恢复)、[模块边界](#模块边界) | G5只实现独立Capacity Admin；G7实现Deadline Watchdog -> Runtime Command Gateway -> T7c、T6e authorization/resolution audit、Command/Trace properties与projection outbox |
| I11 | Contract Pack、managed runtime toolchain/launcher、测试模型、发布门禁、静态 absence baseline 与开发期交付 | [测试策略与模型验证](#测试策略与模型验证) | [Compiler Conformance Toolchain](#compiler-conformance-toolchain)、[开发期实施顺序](#开发期实施顺序)、[开发期直接重构约束](#开发期直接重构约束)、[完整验收标准](#完整验收标准)、[SQLite Execution Profile](#sqlite-execution-profile) | Managed distribution/Launcher proof、Contract Pack、current Gate ownership authority/checker、Sealed Golden、Fixture/Property/Model/Fault、G8 readiness validation、absence/coverage gate |
...
| Capacity payload、publication、权限或活动 head | Capacity Admin Command/Permission/Reason/Denial、revision/hash CAS、Logical Schema、atomic file replace、Watcher/recovery、Admission lineage、Runtime Center 与 fault fixture |
...
3. 若任务涉及 Store，先确认 executable migration 与 Schema Manifest 门禁；若会启动非 test-only Runtime，再确认 stable Launcher/active Managed Node Distribution、与 exact content-addressed release 匹配的 G8 readiness evidence、`local_single_user` deployment profile、静态 absence baseline 与产品 surface coverage manifest。
...
| A3 | dispatch/execution deadline与durable Retry Schedule归G5 T6d；Workflow deadline enforcement归G7 Deadline Watchdog/Gateway/T7c | Safety、Attempt/Retry DDL、T6d、T7c、Runtime Command |
...
| E12 | Safety 与 G8 readiness Supported 边界执行终止性交叉约束，保证合法 Run 一定可原子 root fence | Runtime Safety、SQLite Profile |
...
| S10 | 实施顺序先冻结合同和 DDL/Store，再实现 durable T0/Runtime；test-only bootstrap 与 exact release readiness gate 分离 | 开发期实施顺序 |
...
31. Runtime 只能加载与当前 deployment/runtime/platform/arch/content-addressed release、DDL schema hash、Core build、stable Launcher/managed Node distribution、Node/SQLite binary/compile options/native module、startup smoke、readiness harness/report 和 versioned SQLite Execution Profile candidate 逐项匹配的 G8 readiness evidence；evidence 必须明确 `not_certified`，Safety Ceiling 必须满足固定 Supported 边界与 root-fence 终止性交叉约束。历史 accepted G8 release/evidence 保持不可变；包含 Production Activation entry 的新 production candidate release 必须对其 exact bytes 取得新的 fresh independent full-G8 boundary PASS，历史 PASS 不能替代该 release 的 revalidation。
...
44. Production Registry 可以没有任何 launchable Domain Recipe。Contract Pack、Compiler、Store 与 Runtime validation 使用 test-only synthetic Recipe/Definition；它们只存在于隔离测试 root，不得发布到 Production Registry 或形成创建入口。
...
47. Production `node_service` 只能由稳定 Icarus Runtime Launcher 启动项目托管的 exact Node distribution。launchd、setup service 和 restart script 不得直接执行 `command -v node`、`/opt/homebrew/bin/node`、`/opt/homebrew/opt/node*` 或其他 moving system path；managed runtime 缺失、版本/hash 不匹配时必须 fail-closed，禁止回退系统 Node。Agent Container 的 VM 镜像身份由独立镜像门禁负责，不进入本机 Core launcher 或 G8 readiness identity。
...
interface G8ReadinessSupportedLimits {
  max_scopes_total: number;
  max_nodes_total: number;
  max_edges_total: number;
  max_attempts_total: number;
...
`G8ReadinessSupportedLimits` 作为 `local_single_user_g8_readiness@1` harness payload 的 closed `supported_limits` 字段发布，并由 harness hash 与 implementation source-tree hash绑定；它不是 `RuntimeSupportedLimits` 发布物，不包含 certification key，也不得产生 certified 声明。

Production v1 固定发布 `local_single_user_safety@1`。下列值是首个目标 Safety Profile，不因机器空闲、业务配置或运行中 reload 改变；G8 按固定 representative transaction 验证与这些原子事务相关的 Supported 边界：
...
`local_single_user_safety@1` immutable；调整任何字段必须发布新 version/hash。已有 Workflow/Run 始终使用创建时 pinned Safety，不因 reload 收紧或放宽。新 Safety 调低只影响新创建对象；调高必须先发布覆盖新边界的 versioned readiness harness/report，否则 Production loader 拒绝。Test-only bootstrap 可以发布更小的独立 profile，但不得复用 production ref/status。

配置载体固定分离：immutable Safety 位于 `src/workflow-runtime/contracts/safety/local_single_user_safety@1.json`，checked-in Capacity bootstrap baseline 位于 `config/workflow-runtime-capacity.json`，Production 活动 Capacity publication 位于 `data/workflow-runtime/workflow-runtime-capacity.json`，immutable SQLite Profile 位于 `src/workflow-runtime/contracts/sqlite/local_single_user_sqlite@1.json`。checked-in baseline 只用于 fresh deployment genesis，不是运行期活动文件；setup/upgrade 不得覆盖已有活动 publication。三者都先 strict parse/closed-schema/hash 再使用；Capacity watcher 只能从活动 publication 原子发布完整 validated snapshot，Safety/SQLite 文件变更必须走 Publish/version gate 而不是文件监听。
...
Capacity 是 deployment-level control plane，不是 Workflow/Plan/Recipe Policy，也不是 `WorkflowRuntimeCommand` 的第七种 target。Production v1 的普通修改只能由服务端认证后的 `human:local-owner` 发起并要求独立权限 `runtime.capacity.manage`；Feature Service、Automation、Workflow、Executor、Card action 和业务 API 均不得获得或代理该权限。CLI、部署工具和 Runtime Center 只是可信 `entrypoint`，不能自报 Actor、Session、roles 或 permission；代表 Human 调用时仍记录 Human Actor 与真实 Session/entrypoint。`system:production-activation` 只有在 fresh DB 不存在 Capacity head 时才持有绑定 `initial_provisioning + exact Core release/config hash` 的一次性 genesis grant，不能修改已存在 deployment。

活动文件不是裸 `DeploymentRuntimeCapacity`，而是唯一关联审计 revision 的 closed publication envelope：

```ts
interface DeploymentRuntimeCapacityPublication {
  format: 'icarus.deployment-runtime-capacity-publication/1';
  deployment_profile: 'local_single_user';
  capacity_revision: number; // 从 1 开始并严格递增
  capacity_change_id: string;
  previous_config_hash: string | null; // 仅 revision=1 时为 null
  capacity: DeploymentRuntimeCapacity;
  publication_hash: string;
}

type CapacityChangeReasonCode =
...
interface InitializeDeploymentCapacityCommand {
  command_type: 'initialize_deployment_capacity';
  command_id: string;
  idempotency_key: string;
  proposed_capacity: DeploymentRuntimeCapacity;
  reason_code: 'initial_provisioning';
  core_release_hash: string;
  evidence_refs: string[];
}

interface ReplaceDeploymentCapacityCommand {
...
`capacity.config_hash` 是对移除自身字段后的完整 Capacity payload 计算的 domain-separated hash；`publication_hash` 是对移除自身字段后的完整 publication envelope 计算的独立 domain-separated hash。`initialize_deployment_capacity` 不是产品 API，只能由 Production Activation 在 head 不存在时以一次性 genesis grant 调用，并把 exact Core release/baseline hash 写入 evidence；head 已存在时稳定拒绝，不能退化为 replace。普通修改的 `reason_text` 是必填、受长度限制的审计 Value，不得包含 Credential/Secret；Reason Catalog 固定每种 reason 是否要求 evidence。修改只接受完整 snapshot，不提供 field patch、环境变量 override 或产品模块文件写接口。`rollback` 只是以旧内容创建新 revision，仍必须通过当前 transition validation，不能降低 `minimum_free_disk_bytes` 或绕过其他 increase-only/closed-schema 约束。

Capacity Admin Gateway 复用统一 Authentication Session、Actor/Delegation 解析、idempotency domain 与 immutable Invocation 基础设施，但使用独立 Command Schema、Permission/Reason/Denial Catalog 和事务，不受 Recipe/Definition `WorkflowCommandPolicy` 控制。denial code 至少固定为 `permission_denied | actor_kind_denied | capacity_already_initialized | capacity_snapshot_invalid | capacity_transition_invalid | expected_capacity_revision_conflict | expected_config_hash_conflict | capacity_change_in_progress | idempotency_conflict | audit_unavailable | publication_failed`。OS owner/mode 只是 defense-in-depth；Watcher 不得把“能写文件”解释成应用层授权。

Normative Logical Schema 增加：

```text
runtime_capacity_head
  - singleton_key                    CHECK singleton_key = 1
  - current_capacity_revision/current_change_id
...
runtime_capacity_admin_invocations
  - id/command_id/invocation_no/submitted_request_hash
  - actor_ref/actor_kind/auth_session_ref/entrypoint/delegation_chain_ref
  - required_permission/authorization_result/execution_result/denial_code
                                     prepared | applied | denied | conflict |
                                     duplicate | failed
...
runtime_capacity_change_events
  ...
  - change_id/command_id/capacity_revision
  - event_type                       prepared | file_installed |
                                     head_committed | watcher_published |
                                     recovered | failed | unauthorized_file_rejected
...
UNIQUE(assigned_capacity_revision) WHERE assigned_capacity_revision IS NOT NULL
UNIQUE(assigned_change_id) WHERE assigned_change_id IS NOT NULL
```

Command Header 保存 canonical request/result；每次 authenticated invocation 都追加不可变的 prepared/applied/denied/conflict/duplicate/failed 审计。`prepared`只允许认证、授权且首次通过CAP1的Invocation：CAP1写入`decided_at_ms`，`applied_at_ms`保持null，之后CAP2-CAP4不得更新或finalize该Invocation。CAP4的`watcher_published` Event和Command canonical result共同表达最终应用完成；同一request后续retry追加新的`duplicate` Invocation并返回Command canonical result，不把初始`prepared`行改成`applied`或`duplicate`。`runtime_capacity_change_events` 是 append-only hash chain，不更新或删除既有事件；Capacity Invocation同样禁止UPDATE/DELETE；`runtime_capacity_head` 是唯一可变 CAS pointer。`proposed_capacity_json` 必须由 closed schema 验证并按 canonical bytes 保存，不能保存开放部分 patch。Executable DDL 必须为 head/current/pending、Command finalization、Invocation result和immutability、事件 hash chain 建立 CHECK/FK/UK/trigger，并把这些对象纳入 Schema Manifest、backup、integrity 和 retention root。

SQLite 与文件系统之间没有跨介质原子事务，因此“原子发布”固定解释为单文件原子替换加 durable journal/recovery，而不是虚构一次 DB+filesystem commit。协议固定为：

1. **CAP0 Authenticate/Validate**：Gateway strict parse closed Command，服务端解析 Actor/Session/entrypoint；initialize 要求一次性 genesis grant、head absent、`initial_provisioning` 与 exact release evidence，replace 要求 `runtime.capacity.manage`、reason/evidence、完整 snapshot hash、cross-field constraint 与相对当前 snapshot 的 transition constraint。认证失败只进入独立 authentication security log，不能伪造 Command Actor；认证成功后的授权或验证失败仍追加 Invocation，且不建立 pending change。
2. **CAP1 Prepare**：`BEGIN IMMEDIATE` 内，initialize 以 `head does not exist` CAS 并分配 revision 1，replace 按 `(expected_capacity_revision, expected_config_hash, head.row_version, pending_change_id IS NULL)` CAS 并分配严格递增 revision；两者都写 Command Header、`authorization_result=allowed/execution_result=prepared/decided_at_ms=txn_now_ms/applied_at_ms=null`的不可变初始Invocation、唯一 change id、canonical snapshot、pending head 和 `prepared` event 后提交。并发 initialize/stale replace 全部不改变 head，返回稳定 conflict；CAP4完成前相同key/request按pending recovery继续唯一publication，不追加`duplicate`；CAP4已有canonical result后，相同 idempotency domain/key/request 返回 canonical 原结果并追加新的 duplicate Invocation。prepared只适用于该authenticated+authorized CAP1 initial Invocation，不能用于denied/conflict/failed/duplicate。
3. **CAP2 Install File**：唯一 `CapacitySnapshotPublisher` 只从 CAP1 durable record 生成 envelope，在活动文件同目录使用 exclusive temp、限制权限、写完整 bytes、`fsync(file)`、atomic rename、`fsync(directory)`；任何产品模块、Runtime Center、setup upgrade 或 Watcher 都不能直接写该路径。成功后追加 `file_installed` event。
4. **CAP3 Commit Head**：短事务重新校验 pending change、旧 head、磁盘 publication bytes/hash 后原子推进 current revision/change/config/publication hash、清空 pending 并追加 `head_committed`。文件已替换但 head 未提交时，Watcher 必须等待/拒绝该 envelope，不能抢先生效。
5. **CAP4 Watcher Publish**：Watcher strict parse完整 envelope，并从 Runtime Store 验证 revision/change id/config/publication hash 与 committed head 全部相等后，以单个 immutable in-memory pointer 发布 snapshot；成功追加 `watcher_published` 并 finalise Command canonical result，但不更新CAP1 prepared Invocation。Scheduler、Wait/Signal、Outbox 和 Blob Coordinator 每次 admission/allocation 只读取一次该 pointer，不能跨 snapshot 拼接字段。

Recovery 必须覆盖每个 CAP 边界：CAP1 后 crash 重试同一 publication；rename 后、CAP3 前 crash 时 Watcher 不接受未 committed 文件，Recovery 根据 pending journal 完成 head 或恢复最后 audited envelope；CAP3 后、CAP4 前 crash 时启动 Watcher 从 committed head 重放。活动文件与 audited head 不匹配、revision 未知、hash 虽合法但无 change record，均追加 `unauthorized_file_rejected`，保持最后已发布 snapshot并由唯一 Publisher 恢复 audited envelope；cold start 无可验证 head/snapshot 时禁止新 admission并 fail closed，不能把磁盘文件自动提升为授权事实。Runtime DB/audit 不可用时新修改一律拒绝。Production upgrade 必须保留现有 head、audit 和活动文件，不能用 checked-in baseline 隐式重置。

Capacity Admin CAP1 Invocation repair选择并闭合了真实的immutable prepared lifecycle：历史Database Schema `4`及其全部合法`applied | denied | conflict | duplicate | failed`行作为provenance逐字段保留；current Database Schema `5`增加`prepared`并以CHECK/trigger限制为上述唯一CAP1组合，提供Schema 4到5的保数据原子upgrade。CAP2-CAP4没有Invocation transition，CAP4只finalise Command canonical result。独立affected-chain regression发现candidate CHECK会错误拒绝一部分Schema 4合法历史terminal行；current CHECK因此保留完整Schema 4 terminal set并只新增prepared分支，upgrade在安装Schema 5 INSERT guards前copy旧行。Fresh/current Schema 5通过prepared/applied/terminal/duplicate guards保持新写decision chronology、allowed denial-null、禁止applied与exact finalized duplicate约束，历史兼容不构成新写旁路。

该repair及独立回归只修复G5前置Contract/Schema，不实现Capacity Admin Gateway/Publisher/Watcher或任何G5 Runtime。Independent regression现为`PASS`，因此G5恢复为`READY`而不是`DONE`，G6-G9继续`NOT_READY`。Current Capacity machine root/logical artifact/logical delta分别为`sha256:d436710893239f01e53d668c23d5ddcfe1a7e4dbee3c00074bc4cd43871c98a6` / `sha256:b15daf99f68f8447aff1da5a9411460497ae29e7067a3802ac588d790066fe30` / `sha256:ca81abe11e332890bde7420fdf8f040856e8076bba9bbc4a03d15ffedb439e3a`；protocol semantic artifact保持`sha256:0e06b38b98bfd2193dbad24297e75b1422fdcc8f11e297835c87d4baf03aea9b`，fixture corpus为9 positive / 31 negative / 14 fault。`37a9792` candidate root/logical artifact/delta `sha256:12f9fdfe9739b767440b56b0e55fedb431b27c546326da90285e96e1fc2ea15c` / `sha256:8160dea544586e02b3429dcc1bb044a40a330cb69920c4d1d2231fbed28866c5` / `sha256:749bdfe16195a1762427aed9d98ff8e7d9c2633d22b6d8e3d31bf9aeaf9d589c`继续作为candidate provenance；更早G0.10 root/logical artifact/delta `sha256:21d06c2d9d45a47f6ebc68c24b9d0acec29c8ae1726d5387bd38c460a7a0a7ec` / `sha256:5d9e79b5f9330a5111e6f61b8d04164c87839a60d55ea350c0aa87b8b1559e66` / `sha256:e8917c737b1eae0f62abfa2de2dec6dc71875122a763882a46aee34c5c84cae6`仍是Schema 4 predecessor provenance，不是current authority。

上述不可变审计在 `local_single_user` 信任边界内防止产品旁路、并发丢更新和无记录文件生效；拥有主机 root 且能同时篡改 DB、binary 和文件的对手不在 v1 威胁模型内。若未来要求抵抗本机管理员或提供不可否认性，必须把事件 hash chain 外送到签名/远程 append-only audit sink 后重新认证，不能只扩大 Actor Catalog。

G8 readiness evidence 固定绑定 `local_single_user + node_service + darwin/arm64`、content-addressed release artifact、DDL/schema hash、Core build、Runtime Launcher、Managed Node Distribution、Node executable、SQLite binary/compile options、candidate `SQLiteExecutionProfile`、startup-smoke harness/report 与 readiness harness/report。report 必须明确 `certification_status=not_certified`；系统 Node、Electron Renderer/Main、测试 Node、Rosetta/x64 或其他系统不得复用。Electron 只通过 Runtime API/Command Gateway 访问，不得打开 Runtime DB。缺少 evidence、任一 identity 与当前 release 不匹配，或配置超过 harness 的固定 Supported 边界时生产 Runtime 启动失败。

启动时先验证 readiness report 与当前 release、Schema 11、candidate SQLite Profile 和 harness hash exact匹配，再做同名字段比较与下列终止性不等式；它们保证任意被配置允许并成功 materialize 的 Run 都能由一次不可拆 T7 root fence 收敛，不能出现“能创建、不能取消”的合法状态：
...
同名总量还分别要求 `safety.run.max_scopes_total/max_nodes_total/max_edges_total/max_attempts_total/max_waits_total/max_builds_total/max_effect_operations_total/max_map_items_total` 不超过 readiness Supported 对应字段；`safety.scope.max_nodes_per_scope/max_edges_per_scope` 也不得超过 Supported 的 node/edge 总量。`deriveWorstCaseT7Facts/ManifestBytes` 是 Run Protocol versioned pure function，保守覆盖 scopes/nodes/edges/attempts/waits/builds/map slots/effects、close requests、eligibilities、events、固定 ID/hash 编码和 canonical manifest overhead；函数实现进入 readiness implementation source-tree hash，不能由部署配置改写。Compiler 对每个 Plan 计算保守的 `max_reconcile_facts_per_ingress`、最大 source fan-out 和最大 frontier bytes；超过 Safety 或 readiness Supported boundary 时在 materialize 前拒绝。T2b、Map Manifest seal、retry/wait/effect 创建必须先在同一事务预留导致未来 root fence 所需的累计对象额度，再插入对象；不得等 T3/T7 已开始后以“事务太大”为业务错误中断。若配置超过 G8 已验证边界，Production Gate 保持关闭，不能依靠运行时猜测分批 fence。
...
Current修正直接更新单一Working集合中的raw source、逐case隔离snapshot、hand-authored review input、case-input binding、Compiler actual candidate与working review bundle；case count保持40，actual Compiler result只作为comparison input，expected full result/Plan/proof/program bytes/hash全部保持null。Working期间`human_judgment`为not requested，不为每次修正创建Draft v5/v6或重置case judgment。只有current生成连续两轮一致、定向/完整机械测试通过且工作树边界干净后，才允许显式`prepare-rc`冻结一个Review Candidate root并请求一次完整fresh independent review；RC任一绑定输入变化即失效并退回Working。不得在Working阶段创建`GoldenSemanticReview`、approval、signature、seal或`conformance/sealed/`内容。
...
schema_raw_hash = SHA-256(canonical_schema_bytes)
schema_ref = "icarus-generated-schema:" || schema_raw_hash
schema_hash = SHA-256(
  ASCII("icarus:workflow-generated-schema:2\n")
  || UTF8(RFC8785_JCS(schema_json))
)
parameter_hash = SHA-256(
  ASCII("icarus:workflow-generated-schema-parameter:2\n")
  || UTF8(RFC8785_JCS({ generator, ...generator_parameters }))
)
```

`schema_ref`的closed pattern是`^icarus-generated-schema:sha256:[0-9a-f]{64}$`，唯一resolver是persisted exact content row；ref中的raw hash、row中的canonical bytes/raw hash/domain hash/byte length必须全部相等。Plan canonical bytes内嵌完整descriptor，因此`plan_hash`绑定schema bytes、ref、generator和parameter hash；Store还必须使用`icarus:workflow-plan-generated-schema-binding:1\n`把exact`plan_id/graph_run_id/plan_hash/schema_ref/schema_hash/generator/parameter_hash`绑定为`binding_hash`。同一ref不得对应不同canonical bytes或domain hash，同一Plan binding不得被moving pointer替换。

generator parameter shape固定如下，所有对象key按RFC8785参与hash，不允许调用方提供预计算verdict：
...
| `better-sqlite3` | `12.11.1` | Runtime/DDL/release validation identity |
...
Repository 使用内容为 `26.5.0` 的 exact `.nvmrc` 作为唯一 Node version-manager source；CI 通过 `node-version-file: .nvmrc` 加载同一 patch，`package.json.packageManager` 固定 `npm@11.17.0`。本机开发、build、native install 和 Core `node_service` 不依赖用户修改系统 Node，也不依赖 nvm/fnm/Volta、shell profile 或 Homebrew moving symlink，而是使用下列项目托管的 distribution。Agent Container 位于独立 VM，其 Node identity 由 VM image gate 保证，不属于本机 managed runtime、G0.1 或 G8 readiness identity；Electron 内置 Node 只属于 API client，也不是 Runtime Node identity。
...
`~/Library/Application Support/Icarus/bin/icarus-runtime` 是 launchd 唯一允许执行的稳定 Runtime Launcher；它从自身 `realpath` 所在的安装布局推导 runtime root，不读取 `HOME`、`PATH` 或 `ICARUS_*_HOME` 等继承环境来改写 production root，再通过原子 `active-node` pointer 选择上述 content-addressed installation，验证 pointer containment、Distribution Manifest、Node version/executable hash 和 active Core Release binding 后 `exec` managed `bin/node`。Setup 先旁路安装并验证新 distribution，再原子切换 pointer；旧进程继续使用原 binary，重启后才使用新 pointer。Launcher、pointer 或 managed runtime 缺失/损坏时返回稳定 identity error并退出，绝不尝试 `PATH`、`command -v node`、Homebrew、version manager 或 Electron Node fallback。测试把完全相同的 Launcher 安装到隔离临时布局并由其自身路径解析临时 root，不向 production Launcher 暴露 runtime-root override。Launcher source/hash 属于 Core Release artifact、SQLite Profile 与 G9 activation audit。历史 v2 binding 继续只选择其已接受 validation entry；新 production candidate 使用 v3 binding，显式绑定 Core、G8 validation 和 Production Activation 三个 entry 的 relative path/hash。Stable Launcher 只可按 closed selector 选择这三个已绑定 entry，不接受任意脚本路径或 checkout entry。

本地所有最终 lock/native 证据和开发验证统一通过 `scripts/runtime-toolchain.sh exec -- <command>` 进入 managed `PATH`；`setup.sh` 负责在首次使用时自动安装，无需用户预装或切换系统 Node。CI 仍由 `actions/setup-node` 按 `.nvmrc` 提供同版本 Linux Node，并验证 npm/package-lock identity；CI Node executable hash 不复用 darwin/arm64 G8 readiness evidence。
...
- Scheduler admission 必须从同一个 Watcher-published immutable pointer 读取 `capacity_revision/capacity_change_id/config_hash` 与全部 Capacity 值，并把三者一起写入 Admission；相同 `config_hash` 的不同 revision 仍是不同治理事件。Admission 不得只按文件当前 hash 事后补 lineage，也不得在一个 admission 内跨 revision 读取不同字段。
...
2. Schema Manifest 逐表列出 column/type/nullability/default、PK/UK/FK/CHECK/index，并计算 domain-separated `schema_hash`。TypeScript contract、Store query、G8 release/readiness evidence 和 Core compatibility record 必须固定同一 hash；文档清单与 Manifest 不一致时以发布失败处理，不能由 Store 猜测。
3. CI 必须在空的真实文件 SQLite 上先按 `SQLiteExecutionProfile` 设置 database-level `page_size/auto_vacuum`，再按顺序执行 migration、切换 WAL 并关闭连接；重新打开后通过 `PRAGMA integrity_check`、`PRAGMA foreign_key_check`，并逐项验证 `journal_mode/synchronous/foreign_keys/busy_timeout/page_size/auto_vacuum/temp_store/wal_autocheckpoint/journal_size_limit/cache_size/mmap_size/trusted_schema/recursive_triggers/read_uncommitted/locking_mode/query_only`。同时校验 `sqlite_version()/sqlite_source_id()`、排序后的 compile options hash、`better-sqlite3` version/native module hash 与 Node runtime version；任一不匹配都使 DDL/G8 readiness Gate 失败。随后从 `sqlite_schema` 与 `pragma_table_info/foreign_key_list/index_list` 重建 Manifest 并逐字节匹配发布快照。
...
6. 首个可执行 migration 的验收包括本文所有持久化对象，不能只建 Graph happy-path 表；Intake/Creation、Registry/Retention、Value/Blob/typed ownership、Ledger/Claim、Capacity Head/Admin Command/Invocation/Change Event、Workflow/Activation/Transition/Root Finalization、Run/Scope/Node/Fact/Operational Blocker、Inbox/Outbox/Effect、Command/Confirmation/Audit 和 Checkpoint 同属 v1 原子交付边界。Capacity publication fault fixture 必须覆盖 prepare、file fsync、rename、directory fsync、head commit、Watcher publish 各边界以及 direct-file tamper/restart recovery。
...
G3 Registry immutable replay handling stays inside the same G1 Store and G3 persistence boundary. After validating the closed batch and after entering the synchronous `BEGIN IMMEDIATE` transaction, the Store queries the exact derived Value, `(resource_type, ref)`, dependency set, closure ref/hash/member set, and snapshot ref/hash/binding identities before issuing DML. An absent identity may be inserted; an existing identity is reusable only when every immutable row field and canonical payload is exact. Exact resources and closures may therefore be reused while an absent snapshot is added. A fully existing exact batch returns the original derived receipt with `disposition=exact_replay` and executes no DML. `created_at_ms` is installation metadata rather than semantic identity: a replay retains the first committed timestamp and cannot rewrite it.
...
G3.6 Retention / Executor ABI compatibility preflight is the final standalone read-only Publish preflight slice. Its closed input binds an exact Feature Release ref/hash and optional primary Execution Artifact, an exact Registry Snapshot ref/hash, the exact Closure ref/hash/root/member set, every `feature_execution_artifact` and `executor_implementation` in that Closure, one immutable Core Compatibility snapshot ref/hash, the Run Protocol ref/hash/major, Executor ABI ref/hash/major, and the exact Retention Policy plus proposed `published -> feature_release` root/member set. Every ref is an immutable `VersionedRef`; every hash is lowercase SHA-256. The input has no latest/range/alias/fallback, active pointer, launchability inference, Retention Handle id, GC selector, or publication selector.
...
Production v1 compatibility is closed to Registry Schema `1`, current Database Schema `5`, Run Protocol major `1`, and Executor ABI major `1`. Database Schema `5` is the G1.7 Capacity prepared-Invocation repair over the frozen G1.6/Schema 4 Activation failure/replay prerequisite; Schema 4 remains predecessor provenance and neither schema identity implies Activation or Capacity execution. Current Run Protocol identity is `icarus.workflow-contract-pack-catalog-protocols@1.0.0` / `sha256:a648dc9326255b109690cb47d58032775825ae065caf8f7cbb0ef73efcf984f7`; pre-repair identity `sha256:e4947c515a28b3baf6782a980db9c26d32612b3c6acd3cd04348e73bd54ff607` remains construction provenance at `627d0bc`. Executor ABI v1 identity is `icarus.workflow-executor-abi@1.0.0` / `sha256:a111ddc602dfce1894eeb09951fb250dff41984ad2e67c9eb316beb698946902`, calculated over the frozen length-prefixed canonical JSON framing, invocation field set, and accepted/heartbeat/succeeded/failed/cancelled result union. The Core Compatibility hash binds its exact Core release/build, supported major arrays, Registry Schema and Database Schema facts; Snapshot Core/DB hashes must equal that same snapshot. The only accepted supported-major arrays are `[1]` and all Artifact/Executor `runtime_abi_major` values must equal `1`.
...
The first failing phase is deterministic and returns one code in this order:
...
The bounded G3.6 audit originally recorded `PUBLISHER_BLOCKED_BY_SCHEMA`. Database Schema `2` resolved that blocker with the additive, G1-owned `icarus.workflow-publisher-schema-prerequisite/1` physical input while preserving the exact historical G0.6 and G0.10 artifacts. The readiness value `PUBLISHER_SCHEMA_PREREQUISITE_READY` authorized only the G3.7 staged Publisher transaction against the existing generic synchronous Store boundary; the following G3.7 slice now consumes it. Database Schema `2` by itself still does not implement Publish, Artifact build/install, Activation, loader, GC, or G4-G9 Runtime.
...
G3.7 implements only `WorkflowPublisher.publish` through the existing generic synchronous Store transaction boundary. Its closed canonical request binds caller `idempotency_domain + idempotency_key`, one exact approved review, canonical source manifest and Compiled IR v2 Values, the G3.1 publish-foundation input, the complete ordered G3.5 exact release-resource query set, the G3.6 Retention/Executor ABI input, and one target Feature Release. The approved review hash covers reviewer `human:local-owner`, reviewer authentication session, approval/expiry window, source/plan hashes, Execution Artifact ref/hash, Closure ref/hash, and target Feature Release ref/hash. A new request requires the authenticated invocation actor/session to equal the reviewer actor/session and requires `approved_at_ms <= requested_at_ms < expires_at_ms`; an already terminal exact replay may use a later authenticated session because it cannot change the bound result.

The Publisher evaluates G3.1 directly and calls G3.5 for every exact release resource. It calls G3.6 once for Snapshot, Closure, Artifact/Executor, Core Protocol/ABI, and Retention verification; G3.6 continues to call G3.3 snapshot preflight and G3.5 exact query rather than reproducing their SQL or error semantics. Before DML, the Publisher requires the G3.1 candidate set, G3.5 query set, G3.6 Closure root/member and Retention member set, approved review, source manifest, Compiled Plan, Execution Artifact, and target release hashes to agree exactly. Source manifest verification reuses the frozen G3.2A hash/closed-schema semantics without reading source paths; Compiled Plan verification reuses the closed IR v2 schema and `icarus:workflow-graph-plan:2\n` hash. It never compiles authoring source, builds/installs an Artifact, resolves latest/current, or derives a missing identity.

One `BEGIN IMMEDIATE` transaction re-runs Store-backed preflights and atomically performs the staged Publish facts: canonical source/plan/request/receipt/result Values; Publisher command, invocation, and event audit; exact `staged -> published` Registry resource CAS; one `status=staged` Feature Release; its exact ordered release-resource rows; one held `published -> feature_release` Retention Handle and exact member set; and command terminalization. Registry, Release, Closure, Retention, and Value tables remain authoritative. No `workflow_feature_active_releases` row is inserted or updated, and the canonical receipt fixes `active_pointer_changed=false`.

The caller key binds one `domain_request_hash`. The first successful invocation is `applied`; same key and same domain request returns the original canonical receipt as `duplicate`; same key and a different domain request is `conflict`; an accepted, fully FK-bindable request whose composed preflight rejects is durably `failed` with a non-launchable staged release audit, zero release resources, zero Retention root, and zero Registry publication. Invocation and Event numbers remain adjacent per command and both streams use immutable domain-separated hash chains. A crash at any point inside the single transaction rolls back every fact, so retry can apply from the unchanged staged Registry; a crash after commit reopens the real file database and recovery returns the canonical duplicate while appending `recovery_started/recovery_succeeded`. Identity collision or CAS drift aborts the whole transaction rather than creating partial publication.

The current G3.7 Contract Pack is `icarus.workflow-contract-pack-g3-workflow-publisher@1.0.0`, artifact `sha256:4fc65c77265c226b7abcb6f17aeaab3af3e7f7e13d5117705cb3872f3dce6933` after the T6d Run Protocol dependency rebuild. It publishes closed request/receipt/result schemas, deterministic positive/negative fixtures, and all request/review/release/receipt/result/invocation/event domains. G3 Runtime semantics remain unchanged: Feature Release Activation, active pointer, Production loader, Execution Artifact build/install, GC/delete, and Authoring stages other than Publish remain outside this slice.
...
G1.5 delivers the minimum additive physical input `icarus.workflow-feature-release-activation-schema-prerequisite/1`. It advances the current executable Database Schema from `2` to `3` and adds exactly three first-class Activation audit objects:
...
#### G1.6 Schema 4 implementation status

G1.6已经按上述machine handoff完成并重新闭合当时的G1。Historical G1 root为`sha256:6f49451868b7a5cab359d1c21f14f79afbc11b12aa1938039daf5914d9c4d591`，Schema hash为`sha256:f517a5e7bb8b3ea91bb37cd6a68b32898ceb62b9044687a8103808be6852106a`，canonical Schema 4 migration为`sha256:4a8ddeb1f9715399ad96c3bc32efa5e8032a3bd484eaed0159c6a24620c1be43`，Schema 3到4 upgrade为`sha256:5ac263fe3279c61f74ba6314f5df98fff59a8f8b32acfa784d2040421ebaa3cf`，Schema 4 SQLite identity为`sha256:e46f58e49b42ad53e3d744de86b6d8fb6299236258459c35d9ca3affa440932c`。Store在任何3到4 upgrade DDL前以frozen `sqlite_schema` identity验证exact Schema 3，并在同一个`BEGIN IMMEDIATE`中要求Activation command/invocation/event及active-pointer四表全空；失败全部rollback。这些Schema 4 bytes继续作为frozen predecessor provenance。

G1.6只交付schema/source/migration/Manifest/fixtures/Store identity prerequisite，没有Activation service、业务DML或active-pointer mutation。其当时将G3.9提升为`READY`并要求独立Schema 4回归的状态属于历史施工事实；后续G3.9与whole-G3已经完成。G3.8A artifact自身记录的`BLOCKED_BY_G1_6`仍是冻结handoff事实，不因消费完成或Schema 5 repair而改写。

#### G1.7 Schema 5 Capacity prepared-Invocation repair status

Current Database Schema `5`只修复Capacity Invocation的可执行lifecycle，不改变G1.6 Activation relation语义。首个CAP1 allowed Invocation必须是`prepared`、`invocation_no=1`、exact Command request、non-null且不早于request的`decided_at_ms`、null denial/applied time且Command未finalize；Invocation UPDATE/DELETE继续禁止。四个current INSERT guard分别验证prepared唯一组合、拒绝任何Schema 5新写Capacity `applied`、保持新terminal row的decision/denial shape、要求`duplicate`引用exact request与已finalized Command。CAP4只finalize Command和追加`watcher_published` Event，不更新prepared Invocation。

Independent regression current G1 root/schema/dependency manifest/physical identity为`sha256:baa39d55cac34133a29b461466aa450fec59bd2fd6df72334e8b33d1d1619869` / `sha256:49aaee7c8f046cd9a15b3bc5b77fbcf1713be2a1872078941043f5ccdca29024` / `sha256:d08cfaae72c003b11a05cb1fbfa546f7cce7fad9ecb56d0746f33de294b8088c` / `sha256:ba025b32bb028f2ffe5df45d9440cd0a897e0a06c076b10b6f641c265ae02090`。Fresh Schema 5 migration=`sha256:2ead40dc2f1618f87247e9d3bb476266797c38560e1ad0537a6afa6f71a3fbf6`，Schema 4到5 upgrade=`sha256:97479810c2c079d71270d5a714faa4b8fa8ebd6af629ef2f7d772af270c2bb0a`，Schema Manifest artifact=`sha256:c9bce166112023cf5e09d41901938f74efbc69cff36da9428a3c21c3064d8439`，SQLite identity=`sha256:5ee3c119cc6a0e0552e2a6fe45b51c8ffd08ec7acdbac66748978ed0d21fdb0a`。`37a9792` G1 candidate identities继续由实施进度账本保留为provenance，不冒充current。

Schema 4到5 upgrade在验证frozen Schema 4 identity后，以单个`BEGIN IMMEDIATE`重建Capacity Invocation table、复制全部旧行并重建FK/UK/CHECK/index/immutability triggers，再安装Schema 5 INSERT guards。旧Schema 4全部合法`applied | denied | conflict | duplicate | failed`行必须逐字段保留，但新`applied`不能插入且新terminal行必须满足current guards；复制、identity、constraint、foreign-key、integrity或commit fault任一失败都全事务rollback。Fresh create、非空upgrade、Store open/reopen和Manifest introspection必须闭合相同current identity。

#### G3.9 Feature Release Activation implementation status

G3.9已经消费frozen G3.8A并在current Database Schema `5`上保持原Schema 4 Activation语义，状态为`DONE`。Independent regression dependency rebuild后的current machine authority为`icarus.workflow-contract-pack-g3-feature-release-activation@1.0.0`，pack hash=`sha256:871ded236e5e8fead95d28b365f9802792213c16d1f8517caf030ab9cc9865f8`，由closed request/receipt/result schemas、domain separator catalog和隔离的9 positive / 53 negative / 17 fault fixtures组成；G3.8A repair artifact继续保持独立冻结身份，不能充当G3.9 Contract。Canonical request Value是caller claims唯一exact权威，完整绑定caller key、actor/session/time、Feature、target/previous Release、expected pointer、exact G3.6 input、target/previous Retention和schema identities。
...
Schema 4时期的G3.1-G3.9独立整体回归已经完成，G3状态为`DONE`。该历史回归保持当时G1 root、Schema 4 Manifest/migration/upgrade、G3.6/G3.7、frozen G3.8A、G3.9 closed schemas/pack和G2 sealed successor全部identity不变；G3.9连续两轮generate/check的全部member bytes稳定。Schema 5 Capacity repair只级联exact G1 bindings，不修改这些G3业务语义；current affected-chain identities必须由repair candidate及后续独立回归分别验证。
...
Independent regression current G3.1/G3.3/G3.5/G3.6/G3.7/G3.9 pack identities分别为`sha256:3f385f1f91353a19fedfd347c93dbea190c97de0f267697bc6dd7536ebfb6024` / `sha256:65f675da51c030424c0f56ccb28a2490759e438d40624ded3af1e4ad23bde92a` / `sha256:bbc4e2cb402c8058a6412da0ebd5a284c2c7af831453cafaa84738a391c15718` / `sha256:c43416c9ca085553bea5ebf2294f594ce434ee275cb14d4954e11374521278a2` / `sha256:9865cf0aafa37b4f44dc293aea6c59221d0f445c2e67fa87dfe53e6be71c9fbb` / `sha256:871ded236e5e8fead95d28b365f9802792213c16d1f8517caf030ab9cc9865f8`。Frozen G3.8A和G2 sealed trees不在Capacity semantic dependency chain中，保持byte-identical；`37a9792` G3 candidate identities作为provenance留在实施进度账本。

#### G4 Test Bootstrap implementation status

G4已经交付并通过独立whole-gate regression。最初machine authority `icarus.workflow-contract-pack-g4-test-bootstrap@1.0.0`的pack/profile/implementation/isolation identities及7 positive / 21 negative / 13 fault corpus只作为早期construction provenance；后续downstream-safe与T6d rebuild同样在下文按历史阶段记录。Current authority必须精确绑定managed Node `26.5.0`/npm `11.17.0`/distribution/executable、Compiler toolchain/build、current G1/Schema、G2 sealed successor、G3.6/G3.7/G3.8A/G3.9、fixture/Fake/clock和implementation identities；任一ref/hash/version/profile/source drift均拒绝。

唯一装配入口`createG4TestBootstrap`没有缺省或隐式选择路径。调用者必须提供exact profile/fixture refs+hashes、instance key和canonical temporary root；每个实例以exclusive mkdir创建canonical `os.tmpdir()` child，owner marker与device/inode共同绑定root，并只在其下创建`workflow-runtime.db`。relative/lexical alias/path escape/symlink、pre-existing empty/nonempty、cross-instance reuse、permission/root-create失败、Production root碰撞和删除后同路径替换均fail closed；exclusive mkdir竞争失败方不拥有也不得清理胜方root。Store只通过current `WorkflowRuntimeConnectionFactory`的Schema 5 `candidate_development`路径create/reopen，不复制DDL、不绕过PRAGMA或host identity gate。初始化中断会先关闭Store并删除owned root；无法清理时留下machine-readable residual marker。Isolation receipt逐项绑定Profile/fixture/Fake/clock/implementation/Schema/SQLite/isolation hashes、root device/inode及database device/inode，reopen在Factory前拒绝database replacement，并验证fresh Store全部84张业务/Registry/audit表为0 rows、Published Registry与active Release pointer为0，以及Production/Feature/API/Automation ingress、real Adapter、network、Production/user data全部不可达或未触碰。

Virtual Clock固定seed=`g4-virtual-clock-seed-0001`、initial time=`1784764800000`，只能显式单调advance；G4实现与测试不使用`Date.now`、真实sleep或wall-clock fallback。Fake Adapter只接受七条预登记exact hashed invocation并逐字节重放`not_applied/applied_with_receipt/applied_but_receipt_lost/still_running/unknown/cancelled/compensated`，undeclared input、operation key、attempt、outcome或response全部拒绝。Profile固定`not_certified/default_enabled=false/production_acceptance=reject`。仓库仍没有Production loader/startup，因此production build/startup/loader拒绝仅由closed negative Contract `test_bootstrap_profile_forbidden`和静态import/surface absence Gate证明，不新增或假造Production入口。

截至`005241262568135f02ace4de97c3fa3eacb49131`，G4 whole-gate状态曾为`DONE`；pack内`EXIT_CANDIDATE_PENDING_INDEPENDENT_G4_REGRESSION`保留切片生成时的machine事实。该次独立回归覆盖两轮14-member byte stability、19项G4 tests、全部上游G0-G3/Schema/Store/40-of-40 replay、263-file Production inventory和protected trees，并修复mkdir竞争cleanup ownership与database inode replacement两个隔离缺口。本阶段仍只提供G5可显式消费的isolated Store/fixture/Fake Adapter/Virtual Clock环境，不创建Workflow/Run/Activation或写任何Runtime业务表；没有T0-T8、Reconciler、Scheduler、Ledger、Wait/Inbox/Outbox、Runtime Command、Projection、真实Adapter、Feature/API/Automation ingress、Production loader/launchability、Execution Artifact build/install、Retention/Blob GC/delete或legacy fallback/compatibility reader。

#### G4 downstream-extensibility isolation prerequisite

后续G5准备审计发现上述G4 isolation boundary把全部当时Production source file count/hash纳入G4 identity，并把规范预留的G5-G9 module path列为必须不存在。这不是G4的稳定语义：G4必须证明test-only bootstrap/profile不可从Production surface到达、不可被Production选择或默认启用，而不得冻结后续已授权Gate拥有的源码树。该finding按施工治理显式reopen G4；旧pack=`sha256:4aff06c8170ffa533320e4180a76a169e6815217d3d86dacfa5f7f5448e18e9e`、profile=`sha256:7f88ff930cb4b9d9d348d7d6803a54831b146be069dd77318014f13a47390e6e`、isolation v1=`sha256:648dab69be05f5bd9c15637d895ea45433fd7f876bb47ff621e8c4c079e618ad`只作为`0052412`历史construction evidence保留。

Current G4 isolation authority必须满足以下边界：

- `src/workflow-runtime/bootstrap/`下递归发现的全部非测试JS/TS必须与G4声明owned inventory exact相等，implementation artifact逐文件绑定bytes；新增、漏算或修改owned source必须拒绝或改变implementation identity。
- Checker每个独立进程实时枚举`src/electron/assistant/features/setup/scripts`当前非测试source，使用TypeScript AST解析static import/export、literal `require`与literal dynamic import，以relative/root与TypeScript module resolver构造graph。任一非G4-authority source到bootstrap/profile authority的直接或间接路径均拒绝；该全source闭包规则包含未来G5-G9 module，不依赖逐路径白名单。相同六个source roots中的JSON/YAML/shell/plist/HTML也必须实时扫描；只有pack声明的15个exact G4 machine-authority configuration paths可以保存G4 marker，且这些path同时作为graph authority target，不能成为Production导入绕过。
- `package.json`的main/bin/exports/config、Production start/default/dev/build/package/setup/auth scripts、known host roots、Feature/API/Automation sources、上述六个roots中的host JSON/YAML/shell/plist/HTML与`tsconfig.json`不得引用、选择或默认启用G4 authority。Contract/test script可以显式调用checker，但不构成Production默认。
- Isolation artifact只绑定上述稳定policy、ownership paths与零违规结论，不绑定全仓Production source count/hash。新增与G4无edge的合法下游source，包括规范模块`runtime/graph-runtime.ts`，不得改变G4 isolation identity；一旦它import G4或通过其他module间接可达则必须失败。Future module existence本身不是G4 absence violation，其合法性和业务语义由拥有它的Gate machine authority负责。
- 该边界不能用comments、宽泛字符串白名单、跳过整个`src`、固定零计数或依赖`.git`证明；AST graph、结构化package/default检查、host config exact marker检查和owned inventory共同构成machine proof。

`945e21283a46accfd175356cd4640805e420199c` reopen candidate的pack=`sha256:41fa8a427ee935669283fe119e04e1384df6d305eddff7396c4efb816ba3eaa8`，profile=`sha256:1c7249c5a53f658db130447117116919f0f7258abbcddd77e092b528860f7798`，isolation boundary `icarus.workflow-test-bootstrap-isolation-boundary/2`=`sha256:2802b9cc93f91531d45c6ae53da554e077455f1b67271edea3e4fe7357ee0073`，14-member digest=`09c06981b03b40c543303f2b0f93e8b86ebe7cc4a422f84119c4604a789e3df5`。独立回归发现该candidate仍遗漏`src` host config：Production `src/index.ts`可导入选择G4 profile的`src/runtime-config.json`而得到零violation。该回归闭合时pack/profile为`sha256:1136d6d379b6d821175046449ec58cd7ba72e6ef72441be45d522e2a6a56e3ba` / `sha256:15cbda14e581cbfb499dc8c0f20a297d695da9bd59af4051e9e3b72de97a5fcf`；T6d Run Protocol affected-chain rebuild后的pre-Capacity-repair pack/profile为`sha256:fec4f3fa5ee3cee253606ce75d079ffc3a7a77132edc32bb9bbdd3d7f5de6ed3` / `sha256:1797a52cb6bcee6d53734f038098ce651e895013cd2a296fd3666c4b72f79d80`，isolation v2继续为`sha256:883f8f2d4040fd03ea5a49bd38826ce6c37d5556175219b66f335793087f9188`。当时Bootstrap implementation与implementation artifact分别为`sha256:a8bea5690d9e0e66ba93cec4ebb5e8a5d471ed160ecb6371effd6cbe18bf56ad`和`sha256:d0d04a99202f4d7ec86bd9893901db5dd2139aaad731d334408e627be4acb928`。这些值是Capacity repair前provenance；Mutation Gate的downstream-safe约束继续生效。

#### G4 downstream-safe whole-gate independent regression closure

本轮独立回归从clean local `main@945e21283a46accfd175356cd4640805e420199c`开始，parent=`005241262568135f02ace4de97c3fa3eacb49131`；环境为`permission_profile=disabled/unrestricted`、filesystem unrestricted、sandbox `danger-full-access`、approval policy `never`，没有申请approval/escalation、创建或切换worktree、Handoff、push、amend或重写历史，全部Node/npm命令经`./scripts/runtime-toolchain.sh exec -- <command>`执行。修复前先确认两轮14-member bytes与digest `09c06981b03b40c543303f2b0f93e8b86ebe7cc4a422f84119c4604a789e3df5`精确匹配`945e212`冻结证据；修复后两轮全部14个member raw bytes再次逐项一致，digest均为`8266a7a963d7f3a5b59cd0e12cfcb150992bf821e8498359bd60a8b8b07a0e47`。

最终`test:g4`为2 files / 33 tests；独立post-fix mutation matrix为53 assertions，其中24项覆盖六类surface与static import/export-from/literal require/literal dynamic import的笛卡尔组合，并覆盖间接re-export、真实tsconfig paths alias、同进程cache写入/恢复失效、main/bin/exports/config、11类Production scripts、六类host config扩展和source-root imported JSON。`contracts:check`、G3.9 34/34、whole G3 97/97、Schema/Store checks、G1 Activation 5 passed / 15 skipped、G1 Store 20/20、G2 47/47、successor replay 40/40、G0.6 8/8、G0.10 10/10、typecheck和build全部通过。Live graph保持275 source / 38 roots / 9 G4 source authority / 4 owned bootstrap / 0 violation；G0.10、G2 successor sealed、G3.8A、G1 Schema tree identities分别保持`2bf94fb4ec0142bcb5348168525f67b348cedda4`、`cf9270f9ec71fa2134de0987b5fe55b5425e399b`、`a358e690e90294f0ad08ec47992ff9f645df7e6f`、`161d6041bc56368bc3fd821a37f5a6a58a8eea1b`且相对baseline零diff。

该历史G4治理结论为`DONE`，当时曾把G5 Basic Runtime提升为`READY`。Pack中的`EXIT_CANDIDATE_PENDING_INDEPENDENT_G4_REGRESSION`与`NOT_READY_BLOCKED_BY_G4_REGRESSION`是生成时冻结事实，不为关闭Gate伪造改写。本轮没有实现G5-G9、T0-T8、Workflow/Run/Activation业务DML、Capacity gateway、Reconciler、Scheduler、Ledger、Wait/Inbox/Outbox、Runtime Command/Projection、Production loader/current/latest resolver/activation、Execution Artifact build/install、Retention/Blob GC/delete、真实Adapter或Feature/API/Automation ingress。

Capacity Contract/Schema repair独立回归后的G4 exact pack/profile/bootstrap implementation/implementation artifact为`sha256:9bf5c84032f8475ff1879ed88d64b686279b285dee1b02425dcb162e4ce949db` / `sha256:81dd4ed2e116c46b2c4a7b0057242409c4478ca55f2df34676a9e07eb942baaf` / `sha256:28a920856d26325ec14c976a9e1da668c1302e06fd3e2e05abb890dff830f018` / `sha256:74803b081571df002b900dc6ed23b9c7fc83f86bcb8f096a331cf2011805825e`；isolation v2保持`sha256:883f8f2d4040fd03ea5a49bd38826ce6c37d5556175219b66f335793087f9188`。该级联只反映Schema 5/G3 exact inputs和Store identity，不增加Runtime行为；G4 33/33与live isolation 0 violation通过后，G5恢复`READY`但未实现。
...
T6d attempt watchdog and retry timers:
    ...
    automatic timer ingress is G5-owned and needs no Gateway authorization;
    future manual advance_retry_schedule reaches the same schedule-consumption primitive
    only after G7 Gateway authorization and Command Invocation audit;
    never create Workflow deadline Command/Header/Invocation/Event rows
...
T7c cancel ingress:
    require an authorized cancel_workflow/cancel_run Command;
    G7 Deadline Watchdog submits due Workflow deadlines through Runtime Command Gateway
    with the dedicated System Grant and stable key
    ...
    duplicate returns the canonical Header result and appends Invocation audit;
...
Production v1 固定使用 `local_single_user_sqlite@1` candidate：`busy_timeout_ms=5000`、`page_size=4096`、`auto_vacuum=incremental`、`temp_store=memory`、`wal_autocheckpoint_pages=4096`、`journal_size_limit_bytes=67108864`、`cache_size_kib=32768`、`mmap_size_bytes=0`、`trusted_schema=false`、`recursive_triggers=false`、`read_uncommitted=false`、`locking_mode=normal`、`read_only_query_only=true`，并沿用类型中固定的 `journal_mode=wal/synchronous=full/foreign_keys=true`。该冻结 candidate 保持 `certification_status=candidate`，不得原地重标或伪造 `certified`；Launcher/Node/SQLite/source/compile-options/native module/release artifact 的真实 identity 由 exact release startup/readiness report 单独记录并 content-addressed 绑定。

Node identity 必须来自 active `ManagedNodeRuntimeDistributionManifest` 对应的 content-addressed installation。G8 readiness validation 使用 stable Launcher 启动同一 active managed `bin/node` 构建 release 并加载 `better-sqlite3@12.11.1`；G9 startup 重新验证 `realpath(process.execPath)` 位于 active managed installation、executable hash 与 Distribution/readiness report 一致、launchd `ProgramArguments[0]` 为稳定 Icarus Runtime Launcher，且 Launcher/Core Release hash 与 activation audit 一致。仅有 `.nvmrc`、当前 shell `node --version` 或系统默认 Node 恰好相同不构成 Production runtime identity。

`page_size/auto_vacuum` 是建库属性；修改必须新建/迁移数据库并重新执行 DDL、startup smoke 与 readiness validation。其他 PRAGMA 即使 SQLite 允许在连接上修改，Production 也只能通过新 immutable Profile candidate、进程重启和与新 content-addressed release 精确绑定的 evidence 应用，不能 hot reload。`auto_vacuum=incremental` 必须由 Blob/Store Coordinator 执行 bounded incremental-vacuum maintenance，禁止在普通请求事务内做无界 vacuum。
...
Profile loader 必须先验证 numeric 字段为有限 JS safe integer；除显式允许 `mmap_size_bytes=0` 的字段外均为正整数，enum/boolean 为 closed value，再允许上述 interpolation。Connection Factory 还要读取并回验全部 Profile PRAGMA 和 database-level `page_size/auto_vacuum`，并验证 deployment profile、runtime surface、`process.platform/process.arch`、release artifact hash、Runtime Launcher hash、managed Node distribution ref/hash、Node executable hash/version、SQLite version/source id、compile options hash、`better-sqlite3` version/native module hash 与 startup/readiness evidence 完全一致；Production activation 在同一数据卷临时目录运行固定 startup-smoke harness，保存结果 hash/duration。不能只设置 PRAGMA 而忽略 identity fields，也不能自动改写数据库来迎合不匹配的 Profile。Repository 使用 exact `.nvmrc=26.5.0`、`packageManager=npm@11.17.0` 和 CI toolchain 固定 Node/npm patch identity；`package.json engines` 只能表达开发兼容性，不能作为 G8 release identity。Production Core 启动禁止读取系统 Node 或根据当前 PATH/环境变量改写 active distribution/profile。
...
G8 的唯一 transaction readiness matrix 由 `local_single_user_g8_readiness@1` 固定：T3=`route_group`、T7=`mixed_lifecycle`、T8/Root Finalization=`maximum_required_child`；每个 family 各有一个 `supported_limit` 与一个 `beyond_limit` case，共六例。每个 Supported case先 warmup 1 次，再 measurement 5 次；使用 release build、真实隔离文件 SQLite、完整 Production PRAGMA/索引和 production transaction entry，记录 p50/p95/p99/max、WAL bytes、peak RSS 与 affected rows。T8 另保留一次 `all_or_nothing` before-commit fault regression，证明 Cut、Child relation 与 transition 全成或全不变。

harness 的 `supported_limits` closed object 固定 scopes=128、nodes=1024、edges=4096、attempts=4096、waits=512、builds=512、effects=2048、facts/transaction=16384、frontier bytes=16777216、fence scopes/nodes/edges/attempts/waits/builds/map slots/effects=128/1024/4096/4096/512/512/256/2048、T7 derived facts=16384、fence manifest bytes=16777216、map items=256、T8 required children=8。Supported cases 必须保持 T3 fixed point、T7 subtree/root fence 与 T8 required-child commit 的不可拆原子语义，并通过完整 correctness invariant 与 SQLite integrity/foreign-key 回验。

短时明显退化上限固定为 T3 max `1000 ms`、T7 max `2000 ms`、T8 max `1000 ms`。这些上限只判定当前 representative release validation；readiness report 必须明确 `certification_status=not_certified`。任何超限、零 affected rows、identity/shape/sample drift 或 invariant 失败都使 G8 fail closed。

每个 Beyond Limit case 必须在 business transaction callback、`BEGIN IMMEDIATE` 和任何写入前由同一 Supported-limit guard 确定性拒绝，返回 `runtime_supported_limit_exceeded`，记录 `affected_rows=0`，并证明已关闭 baseline database 的 before/after raw hash 相同。不得为拒绝 case 打开写连接或构造异常权威状态。

G8 startup smoke 由 stable Launcher 从 active content-addressed release 的 validation entry 执行：在空隔离 `STORE_DIR` 创建 Schema 11 真实文件 DB，设置并回验完整 Production PRAGMA，执行零行参数化 `BEGIN IMMEDIATE`，close/reopen 后运行 integrity/foreign-key checks，并在 `5000 ms` 内清理 DB/WAL/SHM。readiness 与 startup report 采用 closed schema、domain-separated hash 和 read-only checker；security-sensitive identity/integrity 只使用静态源码、仓库已有测试及 invariant mapping，并记录 `SECURITY_VALIDATION_NOT_RUN`。

## Retry、Pause、Cancel 与 Compensation

- `max_attempts` 包含首次 Attempt；每次 execution retry 或 quality revision 都创建新 immutable Attempt。
- Workflow 在 T0 冻结 `deadline_at_ms = started_at_ms + effective_max_duration_ms`。G7-owned Deadline Watchdog扫描`query:workflow_deadline_due`，经Runtime Command Gateway使用`deadline_enforced | safety_enforced + due_target + cancel_workflow_only`专用System Grant和稳定key `workflow-deadline:<workflow_id>:<deadline_at_ms>`提交`cancel_workflow -> T7c`；T7c验证authorization并写Command Invocation audit。若normal close request先提交则deadline是late command，反之deadline先赢则后续normal completion只审计；duplicate返回canonical结果并追加Invocation。Workflow deadline不进入G5 T6d，也不走业务`on_error/on_local_cancel`。
...
- Manual node skip 与 retry-wait advance 只允许 graph paused 且携带 expected versions；Cancel 不要求先 Pause，而是直接通过 T7c/T7a 的 Close Request CAS 和 subtree fence 原子抢占。Skip 只允许 Published Command Policy 明确授权、Node 尚未 terminal 且不存在 outcome-unknown effect；它发布 `skipped/manual_skip` fact 并按正常 T3 协议收敛，不能伪造 output。Active effect 必须先 fence/cancel/reconcile，不能直接 skip。`advance_retry_schedule`仍映射T6d primitive，但只有未来G7 Gateway完成Actor/permission/Published Policy/state guard/expected version与Command/Invocation audit后才能调用`authorized_manual_retry`入口；G5只实现primitive和automatic timer入口，不能接受预授权boolean、直接manual调用或伪造Gateway。Manual retry只能消费已有schedule、追加同effect-key remediation或重新reconcile，不能重新打开terminal node，也不能绕过原Policy/Safety/Ledger ceiling。
...
Workflow Runtime Command 只控制既有 Workflow/Run/Node/Effect/Operational Blocker，不控制 deployment Capacity。`replace_deployment_capacity` 必须调用独立 Capacity Admin Gateway、`runtime.capacity.manage` 与 revision/hash CAS；不得扩展下列 Workflow Command union、复用 `WorkflowCommandPolicy`、伪造 Workflow target，或由 Feature/Automation delegation 获得 Capacity authority。
...
- `runtime.capacity.manage` 是 deployment-level 专用权限，Production v1 只授予认证 `human:local-owner`；`system:production-activation` 只持有 fresh deployment genesis grant。文件 owner/mode、CLI 可执行权限、Feature Manifest ceiling、Workflow ownership 或普通 Runtime admin 权限均不能推出该权限，所有修改必须经过 Capacity Admin Gateway 和 immutable audit。
...
Capacity 管理位于 Runtime Center 的诊断/管理子页，不增加第五个一级执行视图。页面通过 authoritative Capacity Admin API 展示 current revision/change id/config hash、完整 snapshot、实时 usage/backpressure/over-capacity、pending publication、Watcher 状态与 immutable change history；修改界面只能提交完整 snapshot，并必须展示 before/after diff、预期背压或 Blob over-capacity 后果、必填 reason text 与 evidence 要求。API 固定为 `GET /api/runtime-admin/capacity`、`GET /api/runtime-admin/capacity/changes` 和 `POST /api/runtime-admin/capacity/changes`；POST 只接受 closed `ReplaceDeploymentCapacityCommand`，要求认证 Human Session 与 `runtime.capacity.manage`，并调用 Capacity Admin Gateway。Renderer、Projection Worker、CLI 和部署工具均不得直接写活动文件或 Runtime DB。

Capacity Admin API 不是 Projection API：current/head 与 change result 从权威 Runtime Store/Gateway 读取，history 可以有可重建 projection，但按钮状态、expected revision/hash 和 authorization 必须在提交事务中重验。Projection degraded 不得把未经验证的旧 revision 提交为 current；Gateway 返回 conflict 后 UI 必须重新读取完整 snapshot，不能自动 merge field patch。
...
Rebuild API 只允许有诊断权限的 Human 提交closed `view + rebuild_job_ref` intent，返回幂等 rebuild job ref；Human/API caller不得提交rows、events、export hash、source wrapper或source head。Server-owned Runtime query/export authority必须由configured真实`WorkflowRuntimeStore`实例构造，在可信Runtime read boundary从持久化Event、Run head、Value/hash与当前domain rows冻结exact source head并生成export；任意in-memory Projection Store、caller-supplied row/event/hash或伪造attested wrapper都不能成为production authority。Export object带不可伪造的authority instance identity/proof且冻结后不可变，Projection只接受同一个configured authority实例实际签发的对象。相同job ref必须绑定同一view与同一authoritative export proof；不同view、rows/head/hash/count或export drift均为conflict，不能作为duplicate吞掉。Rebuild删除/替换的只有可重建 Projection generation，不改 Runtime、Trace、Feature domain data 或 Command audit。新 generation 追平 frozen source head、通过event sequence/canonical payload/Value binding、row-count/hash/referential fixture后原子切换；伪造、自算hash、source lineage断裂、reopen tamper或任何失败都继续服务上一 generation并标记degraded。Projection lag/degraded 时列表和详情仍返回最后可信 snapshot及显式 freshness，Command 按钮默认 disabled；用户经详情重新读取权威 target 后仍可提交的命令必须由 Gateway 独立判断，不能由前端绕过。
...
- Workflow lifetime counters/deadline、runtime safety snapshot、supported-limit hash、Admission 使用的 capacity revision/change id/config hash、domain resource claims/fencing token 和 parent/child Workflow lineage。
...
| `registry/production-activation.ts` | exact content-addressed release/readiness evidence、deployment profile、absence/coverage gate、DeploymentActivationBinding/journal/commit pointer 与原子激活恢复 |
| `registry/production-activation-entry.ts` | 新 production release 内唯一专用 activation entry；只消费 closed request 与已绑定 authority，不是 release 外独立 controller |
...
| `runtime/node-execution.ts` | attempt、artifact/evaluation、effect/receipt journal与G5 automatic attempt/retry timer primitive；不创建Runtime Command |
...
| `runtime/operational-blockers.ts` | G5实现blocker create/open集合与Run/Workflow cache一致性；G7实现T6e remediation/integrity restoration、resolution/abandon与last-blocker state restoration |
| `runtime/commands.ts` | G7实现Actor/Delegation、Runtime Command Gateway授权、Deadline Watchdog -> cancel_workflow/T7c、typed command与immutable Invocation/Event audit；G5不得提供预授权或test-only直写旁路 |
| `runtime/capacity-admin.ts` | Capacity Admin closed command、Actor/Session/permission、idempotency、revision/hash CAS 与 immutable audit；不属于 Workflow Command union |
| `runtime/capacity-publication.ts` | 唯一活动 Capacity file writer、publication envelope、file/directory fsync、Watcher atomic pointer 与 CAP recovery；其他产品模块禁止写活动文件 |
...
依赖方向固定为 `contracts <- compiler/registry/store <- creation/runtime <- projection/host adapters`。`contracts` 不 import DB、Feature 或 Runtime；Compiler 只读 immutable contract/registry snapshot，不 import Store transaction；Store 不 import Compiler 或业务 Adapter；Projection/Renderer 不获得 write connection；外部 Worker/Adapter 只能调用 Callback/Broker/Command API。`runtime/capacity-publication.ts` 是 runtime data root 中活动 Capacity publication 的唯一 filesystem writer，但仍只能通过 Store 中 CAP journal/head 驱动，不能成为第二个权威状态源。Host bootstrap/launcher 只能读取 closed Toolchain/Core Release Manifest，不得 import Runtime TypeScript、打开 Runtime DB、读取 Feature resource 或根据 PATH 猜 Node；setup 只能在 fresh deployment 安装 checked-in Capacity baseline，不得覆盖已有活动 publication。CI 使用 dependency test 扫描 import graph，发现逆向依赖、Runtime DB 直写、活动 Capacity 旁路 writer 或 system-Node fallback 即失败。
...
固定 Fixture、Property-based Test 与 Model-based Test 三类必须并存：Fixture 固定 Hash、Error Code、关键案例和历史回归；Property Test 对生成输入检查不变量；Model Test 把真实 SQLite Runtime 的事件序列结果与独立 reference state machine 比较。G5 Reference Model只表达Workflow/Activation/Run/Scope/Node/Attempt/quality-continuation/Edge/Wait/Fact/Ledger、T6d automatic attempt watchdog/retry timer primitive与Operational Blocker create/open/cache语义，事务集合exact为`T0/T0p/T1/T2a/T2b/T3a/T3b/T4/T5/T6a/T6b/T6c/T6d`；它只以negative evidence证明无Gateway时manual T6d入口不可调用，不创建Command row或伪造authorization。G7才增加Deadline Watchdog -> Runtime Command Gateway -> T7c、authorized `advance_retry_schedule` handoff、T6e authorization、resolution/abandon、Recovery与operational restoration。Reference Model不调用生产Reconciler，也不复制Lease/Outbox实现细节，避免同一Bug同时污染实现和oracle。
...
| Time/DDL | 权威时间均为 safe-integer `*_at_ms`；Activation/Finalization/Fact/Blocker/Command 的状态 CHECK、row version、typed FK/exactly-one 与 partial-index 查询覆盖一致；database/connection PRAGMA 与 exact release startup/readiness evidence 逐项匹配 Profile candidate |

所有时间测试使用 Virtual Clock，禁止真实 `sleep()`；Fake Adapter 可确定性返回 `not_applied/applied_with_receipt/applied_but_receipt_lost/still_running/unknown/cancelled/compensated`。Fault Injection 覆盖 managed Node archive download/hash/安全解包、temp install/fsync/no-replace/active-pointer swap、Launcher verify/exec，Capacity CAP1 prepare、temp write/file fsync/rename/directory fsync、CAP3 head commit、CAP4 Watcher publish、direct-file tamper、相同 hash 新 revision与冷启动恢复，intent、dispatch、external apply、receipt、Evaluator decision/feedback envelope/quality-revision Schedule/next Attempt、quality exhaustion detail、Node output、Fact/Event、Close Request、T0p transition provenance、Root Finalization preflight/retry、T8 required-child + Intake/Creation Request + source-activation completion原子提交、Notification intent/delivery failure、Completion Cut、Workflow transition，以及 Blob file fsync/install/directory fsync/DB commit、GC mark/delete/finalize和backup pin/copy边界；G5 fault范围包含Operational Blocker create/open/cache但不包含resolve/abandon。G7 fault范围增加T6e resolve/last-blocker state restore、Command/Invocation/Event原子性、Administrative Abandon request/expiry/confirm/consume与Recovery；SQLite事务内crash必须全回滚，外部effect边界按operation key/receipt/reconciliation收敛。Upgrade fixture覆盖managed Node side-by-side install/rollback、不得覆盖活动Capacity publication、Core Protocol/ABI兼容与拒绝、Prompt Rebase pass/fail、安全section覆盖和旧Run exact snapshot。

CI 分层：普通提交运行 managed toolchain manifest/bootstrap/launcher/launchd conformance（使用临时 runtime home 和本地 archive fixture，不写系统目录）、Contract Pack conformance、Sealed Compiler Golden Bundle、固定 seed、小型 exhaustive、数百组 property、真实 data/store root write boundary、静态 absence baseline、removed surface negative fixtures 与候选目录不可达证明；完整回归扩大 seed、事件长度和 crash point；G8 另运行真实 managed darwin/arm64 Node distribution、stable Launcher startup smoke 与真实 SQLite 的六例 T3/T7/Root Finalization readiness validation，并对同一 source build 重新生成 absence/coverage hash。所有随机失败都必须能由 seed + shrunk event list 完整重放。
...
本文批准即授权从第 1 步开始实施，不再等待额外架构确认。Contract Pack、Executable DDL、Schema Manifest、Sealed Golden Bundle 与 G8 content-addressed release readiness evidence 是对应 gate 必须实际生成并由 CI/validation 证明的 exit artifact，不是批准本文前要伪造的附件，也不能因目标架构已确认而跳过。下列编号表示 capability gate，不表示所有工作必须串行；只有依赖 gate 通过后才能合并依赖它的实现。当前“可直接执行”指可以从 Contract Pack/Spec Stabilization 开工，不表示 Store 或 Production 已提前通过后续门禁。
...
5. **Test-only bootstrap**：提供只允许固定 fixture、Fake Adapter、临时数据目录且不可被Feature/API/Automation/host/Production入口消费的benchmark/bootstrap profile，用于开发Store/T0-T8和生成 G8 readiness data。它不是production profile，必须在production build/startup中fail-closed，不能成为“开发默认配置”。该Gate exact拥有自身bootstrap source并实时检查当前import graph，但不要求后续已授权G5-G9 module path不存在，也不把持续增长的Production source count/hash纳入G4 identity。
6. **Durable Creation 与基础 Runtime**：在 Store/Registry/Compiler 已存在后实现 Task Intake、durable T0/T0p、claims/ledger、State Activation、T1/T2、delegation/system/wait/join/terminal、T3/T4/T5/T6a-c，以及G5-owned T6d automatic attempt dispatch/execution watchdog、execution-retry/quality-revision durable timer与schedule-consumption primitive；G5 model/fault只覆盖automatic路径和“无G7 Gateway authorization不可调用manual入口”的negative evidence。实现Operational Blocker create/open集合与Run/Workflow operational-state cache一致性、effect key/mutable receipt、versioned Outbox Policy、typed adapter与Inbox/Outbox；G5失败路径可以幂等创建open blocker，但不得执行或测试`open -> resolved/abandoned`，不得实现Workflow deadline enforcement、Runtime Command Gateway、预授权boolean、伪造Command/Invocation/Event或test-only直写。Scheduler/Wait/Signal/Outbox/Blob开始消费Live Capacity前必须实现独立Capacity Admin Gateway、唯一Publisher、Watcher immutable pointer、CAP recovery与Admission revision/change/hash lineage。
...
8. **控制、Card、Projection 与恢复**：实现G7-owned Deadline Watchdog，经Runtime Command Gateway使用专用System Grant和稳定key提交`cancel_workflow -> T7c`；实现authorized `advance_retry_schedule`在Gateway完成permission/policy/state guard/Command/Invocation audit后调用既有G5 T6d primitive。并实现pause/resuming/cancel、root coordinator、checkpoint、domain claim handoff/release、完整T6e authorization与source-specific remediation/integrity restoration、resolution Command/Invocation/Event、last-blocker operational restoration、administrative abandon、Recovery、Card Presentation/typed action、Runtime Center Projection/API/deep link、Capacity诊断/管理子页与独立renderer bundle；Capacity UI只能调用authoritative Capacity Admin API，仍只在test-only bootstrap下执行未认证Runtime。
9. **G8 Gate**：完成受影响的 Reference Model、Property/Model/Fault tests 与基础 affected-chain regression；由 stable Launcher/active managed Node 对同一 content-addressed release 完成 Schema 11 startup smoke，并用真实 SQLite 执行固定六例 T3/T7/T8 readiness validation及T8 all-or-nothing fault regression。生成 closed startup/readiness reports并由只读 checker 通过；report 必须为 `not_certified`。
10. **Production Activation**：先构建包含专用 Production Activation entry 的新 content-addressed production candidate release；历史 accepted G8 release及其 startup/readiness evidence 保持不可变。新 release 对 exact bytes 完成 fresh independent full-G8 boundary PASS 前禁止执行 Production Activation。通过后，对该 release build 重新生成并校验 `WorkflowRuntimeAbsenceBaseline`、`ProductSurfaceCoverageManifest` 与 `MigrationCandidateBoundaryManifest`，由 stable Launcher 启动 active managed Node distribution，验证 Launcher/Core Release/Distribution/Node executable/native module/SQLite Profile candidate 与适用于新 release 的 G8 startup/readiness report exact匹配；fresh deployment 以 `system:production-activation` 一次性 grant 从 checked-in baseline 创建 revision 1 genesis audit/publication，existing deployment 则验证并保留原 Capacity head/file，不得重置。随后以 content-addressed `DeploymentActivationBinding`、durable journal 与单一 `active-deployment` commit pointer 激活 Core/Feature Registry pointer 与 Runtime Center Projection generation。Production Recipe inventory 可以为空；为空时验证通用 Intake=`no_route_available`、Runtime Center Workflow/待处理空状态和非 Workflow Trace 正常。存在 Published Recipe 时只验证其标准 Publish/Activate 合同，不增加任何历史候选特例。

Fresh Capacity genesis 的全部前置 Registry/Value authority 必须由同一 production Core release 内 closed、release-owned、content-addressed `CapacityGenesisBootstrapBundle` 提供，不能依赖隐藏预置行、migration seed、test bootstrap或外部布局。Bundle 至少包含三个按固定顺序声明的 static Core-owned Published Schema Resource：自绑定且只作为bootstrap root-of-trust的 Capacity genesis Registry bootstrap meta-schema、closed `icarus.capacity-admin-result/1` revision-1 applied result Schema、closed Capacity genesis evidence Schema；每个member同时绑定artifact path/raw hash、canonical Schema bytes、Registry ref/content hash、canonical Value id、Registry Resource row id、owner Core ref、schema binding和exact dependencies，因而Bundle之外没有dangling schema/value/resource identity。Meta-schema Value与其Resource构成唯一自绑定root；result/evidence Schema Value均绑定meta-schema Resource并显式依赖它。Bundle hash覆盖member顺序和全部上述authority，Core Release Manifest同时绑定Bundle path/hash且release inventory绑定所有member exact bytes。

Fresh deployment 的deployment-specific evidence Value只能由Bundle中的closed deterministic contract生成。首先构造不含request hash、DeploymentActivationBinding hash或任何下游identity的`ActivationAuditAuthority`，其exact输入固定为activation id、`system:production-activation`、authority timestamp、target Core release artifact hash、previous deployment binding或absent以及`fresh_genesis` mode；随后以exact Core release artifact hash、checked-in Capacity baseline config hash、该pre-binding audit-authority hash、`capacity_revision=1`与`purpose=initial_provisioning`生成canonical evidence document、domain-separated content hash和content-addressed Value id。Identity方向固定为`static Schema bytes -> Registry member identities -> Bundle hash -> Core Release hash -> ActivationAuditAuthority hash -> evidence Value identity -> DeploymentActivationBinding intent -> request/final audit/binding identities`；Bundle、Schema或evidence均不得读取、嵌入或反向绑定DeploymentActivationBinding hash，final audit/request也不得反馈改变evidence identity。

Production Activation 的 commit/recovery protocol 固定如下：

- `DeploymentActivationBinding` 是 closed content-addressed object，精确绑定 production Core release/manifest/build、适用的新 G8 startup/readiness evidence、current absence/surface/migration manifests、Feature Registry target pointer aggregate、四个 Runtime Center Projection target generation、Capacity mode/evidence 与 activation audit。任一底层 identity 不一致均 fail closed。
- Production Activation entry 只能从 stable Launcher v3 已逐项校验的 installed managed release 位置运行；entry directory 的 canonical 三层祖先必须精确等于 `runtime-home/core-releases/<release_artifact_hash>`，且该 root 内 `core-production-release-manifest.json` 必须是 Launcher 已校验并与 request/binding 再次 exact 绑定的同一 manifest。不得使用 repository checkout、外部 path、额外目录层级或 fallback 布局替代 installed release root。
- 每次 activation 使用独立 durable append-only journal event chain。事件至少覆盖 `prepared`、可逆 participant prepare、`active_deployment_committed`、post-commit participant roll-forward 与 `completed`；每个事件绑定前序 hash、binding hash、sequence 和 authority timestamp。Journal 不能替代 active pointer，也不能重写历史事件。若`active-deployment`切换已经durable而对应commit event尚未追加，恢复必须先按observed exact target pointer追加唯一`active_deployment_committed`证据，再执行任何participant roll-forward或写入`completed`；所有partial/terminal journal均须通过closed phase order检查并可exact replay。
- 唯一 commit point 是原子切换 `active-deployment -> deployment-bindings/<binding_hash>`。Commit 前旧 binding（fresh deployment 时为 absent）仍是唯一权威；恢复必须确定性撤销可逆 prepare并保持旧`active-core`，不得修改 existing Capacity。Commit 后 target binding 是唯一权威，恢复只能按相同 operation key 幂等 roll-forward，禁止回退到旧 deployment。
- Fresh Capacity genesis 是不可逆 deployment authority，因此只能在 `active-deployment` commit 后作为 idempotent roll-forward 执行；binding 在 commit 前必须同时绑定 exact checked-in baseline、target Core release、CapacityGenesisBootstrapBundle hash、pre-binding ActivationAuditAuthority hash、deterministically derived evidence Value id/hash和预期 revision 1 evidence。Capacity participant 在commit后、CAP1前先用一个deferred-FK短事务按`bootstrap meta-schema Value -> meta-schema Resource -> result Schema Value -> result Schema Resource -> evidence Schema Value -> evidence Schema Resource -> exact dependency rows`固定顺序install或exact-verify Bundle Registry authority，再以独立短事务生成并install/exact-verifydeployment evidence Value，最后才执行CAP0-CAP4；same id与same canonical bytes/hash是幂等reuse，same id任一bytes/hash/owner/schema/dependency差异均fail closed。任何pre-commit路径不得写Bundle/evidence；commit后恢复只允许对同一target重复上述exact install/verify并继续roll-forward。CAP4已形成exact terminal authority时，后续恢复必须只读验证并保持相同head、audit chain、publication、raw file、Bundle/evidence dependencies和canonical result后直接收敛，不得再次创建duplicate invocation或把terminal结果降级为CAP1 pending recovery。Existing Capacity 不执行、不重装也不修补fresh Bundle/evidence authority；它必须在 prepare 与 roll-forward 两次验证 exact Capacity head/audit/publication/file及其既有dependency objects identity且全程零写入，不能降级为 genesis、replace 或 rollback。
- Feature Release 激活只调用标准 Publish/Activate Contract；Projection generation 由可信 Store export确定性计算并在 commit 后切换。Projection generation object 只按`generation_aggregate_hash`寻址并包含projection version、四个exact generation与aggregate hash；`DeploymentActivationBinding`已单向绑定该aggregate，因此generation object不得反向嵌入deployment binding hash。同一exact generation可由多个deployment binding复用且必须保持相同bytes，不能因deployment identity不同产生content-address collision。新 production Core binding 在 pre-commit 只写入非权威 `activation-core` staging pointer，`active-core` 保持previous binding且不发生切换；stable Launcher 仅用该 staging pointer启动新 release 的G8 validation或Production Activation entry。`active-deployment` commit 后才把`active-core`幂等切换到target binding，并只允许完成target Core/Feature/Projection/Capacity convergence。
- CapacityGenesisBootstrapBundle或其member、release manifest/binding schema、activation implementation任一release-owned byte改变，都会形成新的content-addressed successor release并使predecessor G8 PASS只保留为immutable history。该successor的construction Stage exit固定为：Bundle/schema/evidence generator与checker、ordinary positive fresh install/CAP0-CAP4和existing zero-write preservation repository tests、完整affected G0-G9 shadow chain及protected-boundary检查全部通过，ledger与machine authority同步并形成clean atomic commit；随后停止，禁止staging、Production Activation、pointer switching或Capacity mutation，直到controller对exact successor bytes完成fresh independent whole-G8 PASS。
...
| G0.10 Capacity Control-Plane Addendum | G0.1-G0.9 | G2 Compiler | additive current root + Capacity Admin/publication/CAP/Logical Schema delta + coverage/inventory/fixtures |
| G1 DDL/Store | G0.10 | G2 Compiler | migration + Schema Manifest + SQLite fixtures |
| G2 Compiler/Golden | G0.1-G0.9；G0.10 不改变 Plan/Compiler 语义 | G0.10/G1 DDL | sealed Golden Bundle + compiler/toolchain hash |
...
| G5 Basic Runtime | G4 + current T6d ownership authority independent regression + Capacity Contract/Schema affected-chain regression | optional Domain Recipe authoring | exact T0/T0p/T1/T2a/T2b/T3a/T3b/T4/T5/T6a/T6b/T6c与T6d automatic timer/model/fault fixtures + manual-without-G7-authorization negative evidence + Operational Blocker create/open/cache fixtures；无deadline Gateway、Command write、resolution/abandon identity |
| G6 Dynamic/Close | G5 | projection query model | T7/T8/child/compensation fixtures |
| G7 Control/Card/Projection/Recovery | G6 | Runtime Center renderer | Deadline Watchdog -> Gateway -> T7c stable-key/System Grant/audit + authorized manual retry handoff + T6e + resolution Command/Invocation/Event + recovery/last-blocker restoration/administrative-abandon + card/projection fixtures |
| G8 | G7 | optional Domain Recipe release | exact release manifest + startup-smoke report + six-case `not_certified` readiness report + affected reference/model/property/fault regression |
...
### G5/G7 T6e Gate ownership resolution（历史闭合）

`88a61ce07f05f4eeabed9c1aaf1719555c1faa18`记录的不可执行冲突按唯一窄边界闭合：G5 transaction set exact为`T0/T0p/T1/T2a/T2b/T3a/T3b/T4/T5/T6a/T6b/T6c/T6d`，只拥有Operational Blocker create、open权威集合以及Run/Workflow operational-state cache一致性。G5失败路径可以创建open blocker，但不得执行、模拟或测试`open -> resolved/abandoned`，不得生成或冒充T6e implementation identity。完整T6e authorization、source-specific remediation/integrity restoration、resolution Command/Invocation/Event、last-blocker state restoration、administrative abandon和Recovery exact归G7，Runtime Command Gateway不下沉G5。

当时的current construction governance由`src/workflow-runtime/contracts/governance/workflow-runtime-gate-ownership@1.json`机器化，identity=`sha256:36289416db3c8898d9b50c04c5ad43fc6b74ef53bbd3a3c99f9d5f5b72786fa8`。它绑定当时transaction protocol=`sha256:7c55b3eff2f29e5dfcbb057d5ff014697ba2e9a421287afa19ec850540cce5f0`与command protocol=`sha256:b12b07b29e9335593c969033c133d221b244798fc079db5fb398b23fbae10789`；这些值现在只作为`627d0bc`前历史provenance保留。T6e的`authorized_runtime_command`、`command_invocation_and_runtime_event`、五类remediation mapping、原Schema 4 resolution FK/CHECK与cache trigger在current Schema 5中保持同一不变量。

该authority是新的`current_construction_gate_ownership`治理身份，只声明未来G5/G7施工/fixture ownership，不改写G0.4/G1历史artifact，不进入G4 pack或Runtime运行时authority，也不授权任何G5业务实现。Authority payload中的`G5_OWNERSHIP_EXIT_CANDIDATE_PENDING_INDEPENDENT_REGRESSION`、`implementation_authorized=false`与`next_required_gate=independent_ownership_and_affected_chain_regression`是生成时冻结的施工事实，不因治理关闭而伪造改写。基于`d9119cb2f6d94ff20a89c89c4c2ef6c2d43f71d8`的独立ownership/affected-chain regression现已`PASS/DONE`，G5 Basic Runtime治理状态为`READY`；这只授权下一独立G5任务按上述窄边界施工，不授权本次回归实现任何Runtime行为。
...
### G5 T6d / Runtime Command ownership resolution

`627d0bc483a971f0d5bdbd59c7fb40c994f90097`记录的T6d blocker按“拆分T6d/G5 exit，Gateway-bound workflow deadline留在G7”闭合。Current T6d identity为`sha256:3d5474096d89fbd723e34e0d2f9d1dadd1b955b5fc36ff447d026257852cac79`，只包含attempt dispatch/execution watchdog、execution-retry/quality-revision timer、`attempt_timeout_fence_and_fact`、`cancel_reconcile_or_compensation_effects`、`schedule_consumed_and_exact_next_attempt`与`node_retry_wait_to_active`，并保留CAS、unique timeout/schedule和duplicate timer语义。`workflow_deadline_current_run`、`stable_workflow_deadline_t7c_command`、`stable_workflow_deadline_command_key`与`late_deadline_command`不得重新进入T6d；T6d还显式禁止Workflow deadline Command创建、Command/Invocation audit写入与绕过Gateway的manual retry。

Workflow deadline enforcement完整归G7：Deadline Watchdog消费既有`query:workflow_deadline_due`，Runtime Command Gateway消费`query:command_idempotency_lookup`，以`system:deadline-watchdog` domain、稳定key `workflow-deadline:<workflow_id>:<deadline_at_ms>`和`deadline_enforced | safety_enforced + due_target + cancel_workflow_only`专用System Grant提交既有`cancel_workflow -> T7c`。T7c要求`authorized_cancel_command`，原子写`command_invocation_audit`，duplicate返回canonical Header结果并追加Invocation，late只审计而不改写winning close。`advance_retry_schedule`继续映射T6d，但command protocol机器化要求G7 Gateway先完成authorization/policy/state guard与Invocation audit，再调用G5-owned `consume_existing_retry_schedule` primitive；G5测试不得伪造Gateway。

T6d repair时command protocol=`sha256:43cc8ef247fcba4bac5e9fccdd654fd393928120755a6405bad232043f0c94ba`，ownership authority=`sha256:0d1e0ffda6f17c637616192e3ddcbf51af55207cd011672317b3a7eb231c5e8e`，状态=`T6D_OWNERSHIP_EXIT_CANDIDATE_PENDING_INDEPENDENT_REGRESSION`。该pre-Capacity-repair authority执行4 positive / 40 negative fixture oracle，绑定source/generated一致、closed field set、artifact raw bytes/tree digest、G5/G7 exact ownership、T6d/T7c/System Grant/manual handoff、query owner、安全矩阵以及当时不变Schema 4/T6e边界。Schema 4 G1 root/schema/manifest `sha256:6f49451868b7a5cab359d1c21f14f79afbc11b12aa1938039daf5914d9c4d591` / `sha256:f517a5e7bb8b3ea91bb37cd6a68b32898ceb62b9044687a8103808be6852106a` / `sha256:87f6787dd5c6382df97120c2e10dc6624143c67efc35e57cb92ea22f16fa666b`现在均为predecessor provenance。

Candidate authority只形成ownership exit candidate，不授权任何Runtime实现；其中的pending status、`implementation_authorized=false`与next-required-gate保持生成时原始事实，不为治理关闭而改写。基于clean `main@fde5c2f82f5153195614007d66cb42984385d768`的独立T6d ownership/affected-chain regression历史上为`PASS/DONE`。Capacity authority随后重开，并已由独立Capacity Contract/Schema affected-chain regression再次闭合；G5 Basic Runtime current状态恢复为`READY`但不是`DONE`，G6-G9继续`NOT_READY`。

该历史独立回归连续两轮复算ownership 3 artifacts、10 affected members/tree digest与4 positive / 40 negative oracle；transaction/command/ownership exact为`sha256:3d5474096d89fbd723e34e0d2f9d1dadd1b955b5fc36ff447d026257852cac79` / `sha256:43cc8ef247fcba4bac5e9fccdd654fd393928120755a6405bad232043f0c94ba` / `sha256:0d1e0ffda6f17c637616192e3ddcbf51af55207cd011672317b3a7eb231c5e8e`，affected tree=`sha256:2efe85fb51757cfa9d4cbe59ec4dfcc02b6c0c95ed1b56638935e11657950b28`。这些是Capacity repair前provenance；transaction/command语义未变，但ownership/affected tree必须随current Schema chain重建。
...
`37a9792` Capacity repair candidate ownership authority=`sha256:4e973c234ddd2c37079b32f33df7780da0b8cbdda3f5eba091d314c7947ef2a8`、affected tree=`sha256:c1e1ff5d27410c497cf614458b7bb273e3aedaa9af879d7dd75423674219ccb5`，继续作为candidate provenance。Independent regression current ownership authority=`sha256:7627be1b36c04cc785e05fa8585a33eb933cb95a8ca98b824b4ae931d0ee343a`，affected tree=`sha256:59c2079ca1d9bfe033d858bd169600e76eee35d93d01bbce1402f2e83a04f965`；transaction/command继续为`sha256:3d5474096d89fbd723e34e0d2f9d1dadd1b955b5fc36ff447d026257852cac79` / `sha256:43cc8ef247fcba4bac5e9fccdd654fd393928120755a6405bad232043f0c94ba`。Authority exact绑定current Capacity root、Schema 5 G1 root/schema/Manifest、G3.1/G3.3/G3.5/G3.6/G3.7/G3.9与G4 pack/profile/implementation；payload的pending status、`implementation_authorized=false`与next-required-gate仍保持generation provenance，治理层PASS只恢复G5施工就绪，不冒充Runtime实现授权或完成证据。
...
`dev_test/fix_test` 资源已作为不可执行 migration candidate 独立保存；是否迁移由 Runtime v1 完成后的独立产品决策决定，不属于本文实现、G8 readiness validation 或 Production Activation。

所有测试使用临时 `DATA_DIR/STORE_DIR`，不得写真实用户数据或把候选资料复制到 fixture root。Pure resolver/Compiler 与 test-only benchmark 可以在没有 release readiness evidence 时运行；任何真实 ingress/Adapter 的 Runtime 必须通过 G8/G9。Production v1 仍固定本地单用户 `node_service + darwin/arm64`；真实文件 SQLite 的 T3/T7/Root Finalization representative Supported/Beyond、WAL、正确性和明显退化上限是发布门禁。若未来部署边界变为多用户、远程服务、多机或分布式 scheduler，必须重新打开 Threat Model、权限隔离、Value 机密性和存储选型，不得沿用本版本 validation 结论。

## v1验收完成后的规范归档与验证生命周期

本文是Dynamic Workflow Runtime v1重构期间的完整施工设计底稿，不是Production Runtime输入，也不是永久日常开发入口。只有同时满足以下条件才允许归档：G0-G9和完整验收基于同一release全部通过；所有Normative字段、状态、事务、恢复和发布规则均已落入TypeScript/JSON Schema/Contract、DDL约束、实现或正反/Model/Fault测试；Sealed Golden、G8 release/startup/readiness evidence、Core Release Manifest和G9 activation audit已发布；Production build/startup不读取本文；最终coverage audit没有发现只存在于Markdown的关键规则。
...
Current G1已使用closed、显式Schema Dependency Manifest，只读取exact required members并记录role/path/format/ref/version/semantic hash/raw hash；G0.6 Gate manifest只作construction provenance，Logical Schema Source、typed relation/query catalog、G0.10 Capacity Logical Schema delta、SQLite Profile、Schema Manifest和canonical migration形成physical identity。普通CI、current G1和Store启动均不扫描Contract目录或维护目录排除列表，新增无关Compiler/Golden/Registry JSON不得改变dependency manifest、physical schema identity或G1 root。施工期旧目录快照只留在Git历史和未来可选archive audit背景，不进入current artifact、普通CI或Runtime startup。最终迁移边界保持不变：Core Release Manifest发布后由其绑定schema/migration/profile ref/hash，Store不得永久依赖施工阶段`FROZEN_G1_1_IDENTITIES`；本阶段不提前实现Core Release、G8/G9或Production identity。
...
- Sealed Compiler Golden Bundle 固定 raw bytes、完整 Registry/Policy/Safety、hand-reviewed diagnostics、canonical Plan 与 source/plan/proof/program hash，覆盖 static lowering、condition、wait、nested subgraph、expand、map 和 policy intersection；production Compiler 不能生成 expected artifact。Crash fixtures按Gate分期：G5覆盖T1-T6c、T6d automatic attempt watchdog/retry timer、Fact/Event与Operational Blocker create/open/cache每个commit前后，并以negative evidence证明manual path无G7 authorization不可调用；不得执行Workflow deadline Command、resolution或abandon。G6增加T7/T8与Root Finalization；G7增加Deadline Watchdog/Gateway/T7c、authorized manual retry handoff、T6e、Command/Invocation/Event、Recovery、resolution/abandon与last-blocker restoration。
...
- Production Registry 中没有 Published Recipe 时，Runtime validation/activation 仍通过；通用 Intake 稳定返回 `no_route_available`，不创建 Workflow/Claim 且不暴露全局选择器，Runtime Center 返回零 Workflow/Recipe 的 ready 空状态。Synthetic Recipe/Definition 只存在于隔离 test-only Registry 且不能发布。
...
- Delegation/system execution retry 与 quality revision 都保留 immutable Attempt history 和 durable Schedule，但使用 closed continuation kind；`needs_revision` 必须原子保存 typed latest feedback envelope 并成为下一 Context Pack 的唯一 revision 输入。`pass` 只发布最终 output，`fail` 不 retry；Node ceiling 得到 `quality_revision_exhausted` + immutable exhaustion detail，Run shared attempt 不足得到 `attempt_budget_exhausted`，Workflow deadline 仍经G7 Deadline Watchdog/Gateway/T7c走global cancel。G5 T6d负责dispatch/execution deadline与eligible time；G7负责Workflow deadline。任何crash/replay都不能分叉parent/child Attempt、重复执行同一Schedule或重复应用deadline Command。
...
- RuntimeSafetyCeilings 按 workflow/run/scope/map/execution/wait/value 等作用域分组、全部显式 finite并进入 hash；首个 Production target 必须逐字段等于 `local_single_user_safety@1`，Pinned Safety immutable versioned，Live Capacity 使用已确认的 `5/256/2048/16 + 20/16/5 GiB` baseline并可原子热调，logical/stored/physical bytes 分开计账，每项均有一对一 Enforcement Matrix。已有 Run 不受新 Safety version影响，Capacity 调低只背压；Safety 调高超过 G8 readiness Supported 边界时启动拒绝。启动必须验证 run total 不超过一次 T7 root fence 的 scopes/nodes/edges/attempts/waits/builds/map slots/effects已验证上限。
- Capacity 普通修改只有认证 `human:local-owner + runtime.capacity.manage` 可以发起；CLI/部署工具/Runtime Center 只是 entrypoint，Feature/Automation/Workflow/Executor 无 delegation path。完整 snapshot 以 expected revision+hash CAS，活动 publication 携带 revision/change id/config/publication hash，唯一 Publisher 执行 file/directory fsync + atomic rename，Watcher 只发布与 committed head 完全一致的单一 immutable pointer。Command/Invocation/Event audit 可回答 who/why/request/result/recovery；相同 config hash 的多次启用由不同 revision/change id 区分，Admission 保存三者。direct-file tamper、DB/文件各 crash point、cold start mismatch、upgrade preservation 和 genesis grant 均有 fail-closed fixture。
...
- G5验证Run/Workflow operational state=`action_required/quarantined`与新建/open Operational Blocker集合双向一致，Workflow business status不被post-terminal blocker覆盖；G5不得关闭或abandon blocker。G7经Runtime Command Gateway执行T6e逐blocker验证和关闭，只有最后一个open blocker消失才恢复operational state=`healthy`；resolution必须原子写真实Command/Invocation/Event，恢复不回退lifecycle/control/fence/ledger/deadline，quarantine不能通过普通remediation降级绕过。
...
- Scheduler 按 durable Run round-robin admission，再按 eligible event/scope manifest/node key 排序；Graph Source 不能声明 scheduler priority，wait 不占 execution slot；每个 Admission 从一个 Watcher-published Capacity pointer 原子读取并保存 revision/change id/config hash，不能跨 snapshot 拼字段或事后按当前文件补 lineage。
...
- G8 对 exact release 使用真实文件 SQLite 执行固定 T3 `route_group`、T7 `mixed_lifecycle`、T8 `maximum_required_child` 的 Supported/Beyond 六例：Supported 各 `1+5` 次并保持不可拆原子语义、integrity/foreign-key 与 max 1000/2000/1000 ms 上限；Beyond 在事务/写入前拒绝、affected rows为0且DB hash不变。T8另通过一次all-or-nothing fault regression。
- Test-only bootstrap 不能进入 Production；任何真实 ingress/Adapter 的 Production Runtime 都必须加载与 DDL/Core/SQLite binary+compile options/Profile candidate/readiness harness 精确匹配的 content-addressed release startup/readiness evidence，且 report 明确 `not_certified`。
- Production v1 只允许 `local_single_user + node_service + darwin/arm64` exact release readiness identity、stable Launcher + active managed Node distribution、稳定 `human:local-owner` 与可信 Publisher/Executor；系统 Node、Electron/其他架构不能复用 evidence，未受信任执行代码和远程多用户模式必须 fail-closed。Activation 对同一 source build 验证 `WorkflowRuntimeAbsenceBaseline`、`ProductSurfaceCoverageManifest` 与 `MigrationCandidateBoundaryManifest`：旧 table/column/row、旧关联字段、artifact/context asset、旧 data root、代码/route/config/Definition/Evaluator/Artifact Contract/fixture/import 全部 absent，removed surface 不得恢复，候选目录不得进入可执行闭包；不允许 archive/tombstone/compatibility reader 或历史导入路径。
