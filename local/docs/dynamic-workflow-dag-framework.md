...
- 每次 activation 使用独立 durable append-only journal event chain。事件至少覆盖 `prepared`、可逆 participant prepare、`active_deployment_committed`、post-commit participant roll-forward 与 `completed`；每个事件绑定前序 hash、binding hash、sequence 和 authority timestamp。Journal 不能替代 active pointer，也不能重写历史事件。
