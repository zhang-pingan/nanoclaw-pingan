...
Command Header 保存 canonical request/result；每次 authenticated invocation 都追加 allowed/denied/conflict/duplicate/failed 审计。`runtime_capacity_change_events` 是 append-only hash chain，不更新或删除既有事件；`runtime_capacity_head` 是唯一可变 CAS pointer。`proposed_capacity_json` 必须由 closed schema 验证并按 canonical bytes 保存，不能保存开放部分 patch。Executable DDL 必须为 head/current/pending、Command finalization、Invocation result 和事件 hash chain 建立 CHECK/FK/UK，并把这些对象纳入 Schema Manifest、backup、integrity 和 retention root。
...
2. **CAP1 Prepare**：`BEGIN IMMEDIATE` 内，initialize 以 `head does not exist` CAS 并分配 revision 1，replace 按 `(expected_capacity_revision, expected_config_hash, head.row_version, pending_change_id IS NULL)` CAS 并分配严格递增 revision；两者都写 Command Header/allowed Invocation、唯一 change id、canonical snapshot、pending head 和 `prepared` event 后提交。并发 initialize/stale replace 全部不改变 head，返回稳定 conflict；相同 idempotency domain/key/request 返回 canonical 原结果并追加 duplicate Invocation。
...
5. **CAP4 Watcher Publish**：Watcher strict parse完整 envelope，并从 Runtime Store 验证 revision/change id/config/publication hash 与 committed head 全部相等后，以单个 immutable in-memory pointer 发布 snapshot；成功追加 `watcher_published` 并 finalise Command canonical result。Scheduler、Wait/Signal、Outbox 和 Blob Coordinator 每次 admission/allocation 只读取一次该 pointer，不能跨 snapshot 拼接字段。
...
G5 implementation启动审计发现当前CAP1与冻结Database Schema `4`存在不可执行冲突，因此G5保持`BLOCKED_BY_SPEC`且不得提交partial Runtime。CAP1要求pending head事务原子写allowed Invocation，但`runtime_capacity_admin_invocations.execution_result`只有`applied | denied | conflict | duplicate | failed`；allowed `applied`强制已有`applied_at_ms`，其余allowed值均是终局冲突/重复/失败，不存在`pending/prepared`。CAP2-CAP4也没有Invocation result transition，CAP4只finalise Command canonical result。实现不得把CAP1伪记为failed/applied后再原地改写；独立repair必须显式reopen受影响Capacity Contract与G1 Schema，选择真实pending lifecycle、延迟Invocation加独立CAP1审计，或新的prepare authorization journal，并重建affected chain。该记录不自行选择repair语义，也不修改既有G0.10/Schema 4 machine bytes。
...
G1.6已经按上述machine handoff完成并重新闭合G1。Current G1 root为`sha256:6f49451868b7a5cab359d1c21f14f79afbc11b12aa1938039daf5914d9c4d591`，Schema hash为`sha256:f517a5e7bb8b3ea91bb37cd6a68b32898ceb62b9044687a8103808be6852106a`，canonical Schema 4 migration为`sha256:4a8ddeb1f9715399ad96c3bc32efa5e8032a3bd484eaed0159c6a24620c1be43`，Schema 3到4 upgrade为`sha256:5ac263fe3279c61f74ba6314f5df98fff59a8f8b32acfa784d2040421ebaa3cf`。Store在任何upgrade DDL前以frozen `sqlite_schema` identity验证exact Schema 3，并在同一个`BEGIN IMMEDIATE`中要求Activation command/invocation/event及active-pointer四表全空；失败全部rollback。Fresh bootstrap直接创建Schema 4。Store启动校验消费frozen artifacts，不重建construction source。
...
G4已经交付并通过独立whole-gate regression。Current machine authority `icarus.workflow-contract-pack-g4-test-bootstrap@1.0.0`的pack hash=`sha256:4aff06c8170ffa533320e4180a76a169e6815217d3d86dacfa5f7f5448e18e9e`；test-only profile artifact=`sha256:7f88ff930cb4b9d9d348d7d6803a54831b146be069dd77318014f13a47390e6e`；bootstrap implementation hash=`sha256:a8bea5690d9e0e66ba93cec4ebb5e8a5d471ed160ecb6371effd6cbe18bf56ad`；isolation boundary artifact=`sha256:648dab69be05f5bd9c15637d895ea45433fd7f876bb47ff621e8c4c079e618ad`。Pack包含closed profile、Fake Adapter invocation/result、isolation receipt schemas，精确fixture set、Fake Adapter/Virtual Clock profiles、implementation source inventory、Production surface absence proof、domain catalog及7 positive / 21 negative / 13 fault fixtures。Profile精确绑定managed Node `26.5.0`/npm `11.17.0`/distribution/executable、Compiler toolchain/build、current G1 root与Schema 4 migration/upgrade/SQLite Profile、G2 sealed successor、G3.6/G3.7/G3.8A/G3.9、fixture/Fake/clock和implementation identities；任一ref/hash/version/profile/source drift均拒绝。

唯一装配入口`createG4TestBootstrap`没有缺省或隐式选择路径。调用者必须提供exact profile/fixture refs+hashes、instance key和canonical temporary root；每个实例以exclusive mkdir创建canonical `os.tmpdir()` child，owner marker与device/inode共同绑定root，并只在其下创建`workflow-runtime.db`。relative/lexical alias/path escape/symlink、pre-existing empty/nonempty、cross-instance reuse、permission/root-create失败、Production root碰撞和删除后同路径替换均fail closed；exclusive mkdir竞争失败方不拥有也不得清理胜方root。Store只通过current `WorkflowRuntimeConnectionFactory`的Schema 4 `candidate_development`路径create/reopen，不复制DDL、不绕过PRAGMA或host identity gate。初始化中断会先关闭Store并删除owned root；无法清理时留下machine-readable residual marker。Isolation receipt逐项绑定Profile/fixture/Fake/clock/implementation/Schema/SQLite/isolation hashes、root device/inode及database device/inode，reopen在Factory前拒绝database replacement，并验证fresh Store全部84张业务/Registry/audit表为0 rows、Published Registry与active Release pointer为0，以及Production/Feature/API/Automation ingress、real Adapter、network、Production/user data全部不可达或未触碰。
