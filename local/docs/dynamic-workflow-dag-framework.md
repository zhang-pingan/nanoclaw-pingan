...
- Fresh Capacity genesis 是不可逆 deployment authority，因此只能在 `active-deployment` commit 后作为 idempotent roll-forward 执行；binding 在 commit 前已绑定 exact checked-in baseline、target Core release和预期 revision 1 evidence。CAP4已形成exact terminal authority时，后续恢复必须只读验证并保持相同head、audit chain、publication、raw file和canonical result后直接收敛，不得再次创建duplicate invocation或把terminal结果降级为CAP1 pending recovery。Existing Capacity 必须在 prepare 与 roll-forward 两次验证 exact head/audit/publication/file identity且全程零写入，不能降级为 genesis、replace 或 rollback。
