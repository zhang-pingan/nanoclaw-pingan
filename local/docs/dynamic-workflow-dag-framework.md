...
- Checker每个独立进程实时枚举`src/electron/assistant/features/setup/scripts`当前非测试source，使用TypeScript AST解析static import/export、literal `require`与literal dynamic import，以relative/root与TypeScript module resolver构造graph。任一非G4-authority source到bootstrap/profile authority的直接或间接路径均拒绝；该全source闭包规则包含未来G5-G9 module，不依赖逐路径白名单。
- `package.json`的main/bin/exports/config、Production start/default/dev/build/package/setup/auth scripts、known host roots、Feature/API/Automation sources、host JSON/YAML/shell/plist/HTML与`tsconfig.json`不得引用、选择或默认启用G4 authority。Contract/test script可以显式调用checker，但不构成Production默认。
...
Reopen后的current pack=`sha256:41fa8a427ee935669283fe119e04e1384df6d305eddff7396c4efb816ba3eaa8`，profile=`sha256:1c7249c5a53f658db130447117116919f0f7258abbcddd77e092b528860f7798`，isolation boundary `icarus.workflow-test-bootstrap-isolation-boundary/2`=`sha256:2802b9cc93f91531d45c6ae53da554e077455f1b67271edea3e4fe7357ee0073`；bootstrap implementation继续为`sha256:a8bea5690d9e0e66ba93cec4ebb5e8a5d471ed160ecb6371effd6cbe18bf56ad`，证明root/database replacement、race ownership、84-table-zero-row、Fake Adapter与Virtual Clock功能语义没有改变。Mutation Gate覆盖downstream-safe source与空`graph-runtime.ts`identity稳定、owned bytes/sibling drift、Production/Feature/API/Automation/host/G5 direct/indirect imports、package start/default、launcher与tsconfig alias。G4当前状态为`EXIT_CANDIDATE_PENDING_INDEPENDENT_G4_REGRESSION`；G5回到`NOT_READY/BLOCKED_BY_G4_REGRESSION`。下一独立任务必须重新完成G4 whole-gate regression，通过并记录关闭证据前不得施工G5。
