...
| I10 | Runtime Command、Capacity Admin、权限、Runtime Center 与 Trace | [Workflow Runtime Command 授权与审计](#workflow-runtime-command-授权与审计)、[Capacity 管理、发布与审计](#capacity-管理发布与审计)、[Runtime Center（运行中心）与 Trace](#runtime-center运行中心与-trace) | [权限与安全](#权限与安全)、[SQLite Execution Profile](#sqlite-execution-profile)、[Outbox、Lease 与恢复](#outboxlease-与恢复)、[模块边界](#模块边界) | G5只实现独立Capacity Admin；G7实现Workflow Runtime Command Gateway、T6e authorization/resolution audit、T7c、Command/Trace properties与projection outbox |
...
| `runtime/commands.ts` | G7实现Actor/Delegation、Runtime Command Gateway授权、typed command与immutable Invocation/Event audit；G5不得提供预授权或test-only直写旁路 |
...
固定 Fixture、Property-based Test 与 Model-based Test 三类必须并存：Fixture 固定 Hash、Error Code、关键案例和历史回归；Property Test 对生成输入检查不变量；Model Test 把真实 SQLite Runtime 的事件序列结果与独立 reference state machine 比较。G5 Reference Model只表达Workflow/Activation/Run/Scope/Node/Attempt/quality-continuation/Edge/Wait/Fact/Ledger与Operational Blocker create/open/cache语义，事务集合exact为`T0/T0p/T1/T2a/T2b/T3a/T3b/T4/T5/T6a/T6b/T6c/T6d`；G7才增加T6e authorization、resolution/abandon、Recovery与operational restoration。Reference Model不调用生产Reconciler，也不复制Lease/Outbox实现细节，避免同一Bug同时污染实现和oracle。
...
| G5 Basic Runtime | G4 + current Gate ownership authority independent regression | optional Domain Recipe authoring | exact T0/T0p/T1/T2a/T2b/T3a/T3b/T4/T5/T6a/T6b/T6c/T6d model/fault fixtures + Operational Blocker create/open/cache fixtures；无resolution/abandon identity |
| G6 Dynamic/Close | G5 | projection query model | T7/T8/child/compensation fixtures |
| G7 Control/Card/Projection/Recovery | G6 | Runtime Center renderer | T6e + Runtime Command Gateway + resolution Command/Invocation/Event + recovery/last-blocker restoration/administrative-abandon + card/projection fixtures |
...
### G5/G7 T6e Gate ownership resolution
...
Current construction governance由`src/workflow-runtime/contracts/governance/workflow-runtime-gate-ownership@1.json`机器化，identity=`sha256:36289416db3c8898d9b50c04c5ad43fc6b74ef53bbd3a3c99f9d5f5b72786fa8`。Checker对G5/G7 transaction和semantic集合实行closed exact验证；missing、duplicate、unknown、cross-Gate ownership或把T6e放回G5均fail closed。它同时绑定并验证冻结transaction protocol=`sha256:7c55b3eff2f29e5dfcbb057d5ff014697ba2e9a421287afa19ec850540cce5f0`、command protocol=`sha256:b12b07b29e9335593c969033c133d221b244798fc079db5fb398b23fbae10789`、G1 root/schema=`sha256:6f49451868b7a5cab359d1c21f14f79afbc11b12aa1938039daf5914d9c4d591` / `sha256:f517a5e7bb8b3ea91bb37cd6a68b32898ceb62b9044687a8103808be6852106a`，并要求T6e继续具有`authorized_runtime_command`与`command_invocation_and_runtime_event`、五类remediation command继续绑定T6e、Schema 4 resolved blocker继续要求真实`resolution_command_id` FK以及insert/update cache trigger不变。
